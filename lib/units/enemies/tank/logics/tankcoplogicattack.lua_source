...
function TankCopLogicAttack.enter( data, new_logic_name, enter_params )
	...
	--print( "TankCopLogicAttack.enter" )
...
function TankCopLogicAttack.update( data )
	...
	if chase then
		...
		elseif my_data.chase_pos then
			...
			local reservation = { position = mvector3.copy( to_pos ), radius = 70, filter = data.pos_rsrv_id }
	...
	if data.pathing_results then
		...
		if path then
			if path ~= "failed" then
				...
				--print( "[TankCopLogicAttack._process_pathing_results] we have a chase path" )
			else
...
function TankCopLogicAttack.chk_should_turn( data, my_data )
	return not ( my_data.turning or data.unit:movement():chk_action_forbidden( "walk" ) or my_data.surprised or my_data.walking_to_chase_pos )
end
...
function TankCopLogicAttack._chk_request_action_walk_to_chase_pos( data, my_data, speed, end_rot )
	if not data.unit:movement():chk_action_forbidden( "walk" ) then
		--print( "[CopLogicTravel. _chk_request_action_walk_to_chase_pos]", data.t )
