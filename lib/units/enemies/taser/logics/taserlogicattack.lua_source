...
function TaserLogicAttack.queued_update( data )
	...
	if my_data.tasing then
		if not action_taken then
			action_taken = CopLogicAttack._chk_request_action_turn_to_enemy( data, my_data, data.m_pos, focus_enemy.m_pos )
		end
...
function TaserLogicAttack._upd_enemy_detection( data )
	...
	for key, enemy_data in pairs( data.detected_attention_objects ) do
		if enemy_data.dmg_t and enemy_data.dmg_t > alert_chk_t then
	...
	if tase_in_effect or tasing and data.t - tasing.start_t < math.max( 1, data.char_tweak.weapon.m4.aim_delay_tase[2] * 1.5 ) then -- we have hit someone or we haefv just started our attempt. do not cancel yet
	...
	--print( "[TaserLogicAttack._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
...
function TaserLogicAttack._upd_aim( data, my_data, reaction )
	...
	if focus_enemy then	-- I have an enemy
		if tase then
			shoot = true
	...
	if aim or shoot then
		...
		if not ( data.unit:anim_data().reload or data.unit:movement():chk_action_forbidden( "action" ) ) then
			if tase then
				if not ( my_data.tasing and my_data.tasing.target_u_data == focus_enemy or data.unit:movement():chk_action_forbidden( "walk" ) ) then
					if my_data.attention_unit ~= focus_enemy.u_key then -- we need to have attention if we are going to tase
						CopLogicBase._set_attention( data, focus_enemy )
						my_data.attention_unit = focus_enemy.u_key
					end
...
function TaserLogicAttack.action_complete_clbk( data, action )
	...
	elseif action_type == "tase" then
		if action:expired() then	--check if the target unit is tased
			if my_data.tasing then
...
function TaserLogicAttack._chk_reaction_to_attention_object( data, attention_data, stationary )
	...
	if reaction < AIAttentionObject.REACT_SHOOT or not ( attention_data.criminal_record and attention_data.is_person ) then
		return reaction
	end
...
function TaserLogicAttack._chk_play_charge_weapon_sound( data, my_data, focus_enemy )
	if not my_data.tasing and ( not my_data.last_charge_snd_play_t or data.t - my_data.last_charge_snd_play_t > 30 ) then
