...
function CopSound:init( unit )
	...
	unit:base():post_init()		--	This needs to be called by the extension that gets initialized last
end

---------------------------------------------------------------

function CopSound:destroy( unit )
 unit:base():pre_destroy( unit )--	This needs to be called by the extension that gets destroyed first
end
...
function CopSound:say( sound_name, sync, skip_prefix )
	...
	self._speak_expire_t = TimerManager:game():time() + 2	-- assume talk lasts for 2 seconds...
end
