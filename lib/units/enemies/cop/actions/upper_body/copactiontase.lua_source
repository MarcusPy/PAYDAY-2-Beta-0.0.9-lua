...
function CopActionTase:init( action_desc, common_data )
	--print( "[CopActionTase:init]", inspect( action_desc ) )
	...
	if not ( attention and attention.unit ) and Network:is_server() then
...
function CopActionTase:on_attention( attention )
	...
	elseif Network:is_server() then
		if self._attention then -- we cannot change attention. need to expire
		...
		if self._client_attention_set or not ( attention and attention.unit ) then
			if self._discharging then
				self._tasing_local_unit:movement():on_tase_ended()
				self._discharging = nil
			end
			...
			self._tasing_local_unit = nil
			self._attention = attention
			self.update = self._upd_empty	--disable the update until we have an attention
			return
		end
		self._client_attention_set = true
	end
	
	-- We have an attention unit. both server and client
	...
	self._tasing_local_unit = nil
	self._tasing_player = nil
	if Network:is_server() then -- the server informs all clients to raise the weapon. but the "fire" decision has to be taken by the client owning the attention unit
		self._common_data.ext_network:send( "action_tase_start" )	-- this means raise the taser gun but don't shoot yet
		if not attention_unit:base().is_husk_player then -- the server can shoot at anyone except player husks
	...
	elseif attention_unit:base().is_local_player then -- I am a client and can take the decision to start tasing the local player
		self._shoot_t = TimerManager:game():time() + shoot_delay
		self._tasing_local_unit = attention_unit
...
function CopActionTase:on_exit()
	...
	if Network:is_server() then
		self._ext_movement:set_stance_by_code( 2 ) -- "hos"
	end
...
function CopActionTase:update( t )
	...
	if fwd_dot > 0.7 then
		...
		self._modifier:set_target_y( target_vec )
		--Application:draw_cylinder( shoot_from_pos, target_pos, 2, 0,0,1 )
	else
		if self._modifier_on then
			self._modifier_on = nil
			self._machine:allow_modifier( self._modifier_name )
		end
		if self._turn_allowed and not ( self._ext_anim.walk or self._ext_anim.turn or self._ext_movement:chk_action_forbidden( "walk" ) ) then
	...
	--[[if target_vec then
		local brush = Draw:brush( Color( 0.3,0,1,0 ) )
		brush:cylinder( shoot_from_pos, target_pos, 0.5 )
	end]]
	if self._ext_anim.reload or self._ext_anim.equip then	-- we are reloading/equipping
	...
	elseif self._shoot_t then
		if target_vec and self._common_data.allow_fire and t > self._shoot_t and t > self._mod_enable_t then
			if self._tase_effect then
				World:effect_manager():fade_kill( self._tase_effect )
			end
			self._tase_effect = World:effect_manager():spawn( { effect = Idstring( "effects/payday2/particles/character/taser_thread" ), parent = self._ext_inventory:equipped_unit():get_object( Idstring( "fire" ) ), force_synch = true } )
			...
			if self._tasing_local_unit and mvector3.distance( shoot_from_pos, target_pos ) < self._w_usage_tweak.tase_distance then
				...
				if not record or record.status or self._tasing_local_unit:movement():chk_action_forbidden( "hurt" ) then
					...
					local vis_ray = self._unit:raycast( "ray", shoot_from_pos, target_pos, "slot_mask", self._line_of_fire_slotmask, "ignore_unit", self._tasing_local_unit, "report" )
					if not vis_ray then
						self._common_data.ext_network:send( "action_tase_fire" )	-- Shoot the taser gun
						local attack_data = {
