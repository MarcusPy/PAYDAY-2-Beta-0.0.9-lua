...
function CopActionIdle:init( action_desc, common_data )
	--print( "[CopActionIdle:init]", inspect( action_desc ) )
	
	if action_desc.non_persistent then
		return	-- do not keep this action. It was only started to interrupt other actions
	end
	...
	if res == false then	-- failed redirect
	...
	if self._attention then
		...
		if self._look_trans then
			...
			if prog > 1 then
				...
				--Application:draw_cylinder( look_from_pos, look_from_pos + target_vec * 500, 2, 0,0,1 )
				...
				if mvec3_dot( target_vec, self._common_data.fwd ) < 0.2 then
					--Application:draw_cylinder( look_from_pos, look_from_pos + target_vec * 500, 1, 0, 0, 0 )
					ik_enable = false
				end
				--Application:draw_cylinder( look_from_pos, look_from_pos + look_trans.start_rot:y() * 500, 1, 0,1,1 )
			end
		else
			if mvec3_dot( target_vec, self._common_data.fwd ) < 0.2 then
				--Application:draw_cylinder( look_from_pos, look_from_pos + target_vec * 500, 1, 0, 0, 0 )
		...
		if ik_enable then
			--Application:draw_cylinder( look_from_pos, look_from_pos + target_vec * 500, 2.5, 0,1,0 )
			...
			if self._turn_allowed then
				...
				if not ( active_actions[1] or active_actions[2] or queued_actions and ( queued_actions[1] or queued_actions[2] ) or self._ext_movement:chk_action_forbidden( "walk" ) ) then
...
function CopActionIdle:on_attention( attention )
	...
	if attention then
		...
		self._look_trans = { start_t = TimerManager:game():time(), duration = duration, start_rot = start_rot }
