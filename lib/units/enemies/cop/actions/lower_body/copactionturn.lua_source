...
function CopActionTurn:init( action_desc, common_data )
	--print( "[CopActionTurn:init]", common_data.unit, inspect( action_desc ) )
	...
	--self._start_yaw = common_data.rot:yaw()
...
function CopActionTurn:update( t )
	if not self._ext_anim.turn and self._ext_anim.idle_full_blend then
		--local end_yaw = self._common_data.unit:rotation():yaw()
		--print( "[CopActionTurn:update] expired dif", end_yaw - self._start_yaw, "self._angle", self._angle, self._common_data.unit )
		self._expired = true
	end
