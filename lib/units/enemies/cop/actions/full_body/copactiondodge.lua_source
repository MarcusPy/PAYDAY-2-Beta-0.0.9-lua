...
function CopActionDodge:init( action_desc, common_data )
	--print( "[CopActionDodge:init]", common_data.unit, inspect( action_desc ) )
	...
	--self._expired = nil
	self._timeout = action_desc.timeout -- only server. used by AI
	...
	if redir_res then
		--print( "[CopActionDodge:init] success", redir_name, redir_res:s() )
...
function CopActionDodge:update( t )
	if self._ext_anim.dodge then
		...
		if self._rot_transition then
			...
			--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + self._common_data.fwd * 200, 6, 0, 0.5, 0 )
			...
			--print( "rot_prog", rot_prog )
			if rot_prog > 1 then
				new_rot = self._rot_transition.end_rot
				self._rot_transition = nil
			else
				--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + self._rot_transition.end_rot:y() * 200, 2, 1, 1, 1 )
				new_rot = self._rot_transition.start_rot:slerp( self._rot_transition.end_rot, rot_prog )
			end
			--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + new_rot:y() * 200, 8, 0, 1, 0 )
		else
...
function CopActionDodge:_determine_rotation_transition()
	...
	--print( "dodge_yaw", end_rot:yaw() )
	--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + end_rot:y() * 200, 2, 0, 0, 0 )
	...
	--print( "end_rot", end_rot:yaw() )
	--Application:draw_cylinder( self._common_data.pos, self._common_data.pos + end_rot:y() * 200, 2, 1, 1, 1 )
