...
function CopLogicTrade.hostage_trade( unit, enable, trade_success )
	...
	if enable then
		...
		managers.hud:add_waypoint( wp_id, { text = text, icon = "wp_trade", position = unit:movement():m_pos(), distance = is_win32 } )
		...
		if trade_success then
			...
			unit:interaction():set_contour( "standard_color", 1.0 )
			...
			--unit:interaction():_set_contour( "selected_color", 0.0 )
		end
	end
end
...
function CopLogicTrade.on_trade( data, trading_unit )
	...
	if flee_pos then
		...
		if data.unit:anim_data().hands_tied or data.unit:anim_data().tied then -- hands_tied for cop, tied for civilian
...
--[[
function CopLogicTrade._cancel_flee_attempt( data, my_data )
	if my_data.flee_path then
		my_data.flee_path = nil
	elseif my_data.walking_to_flee_pos then
		local new_action = { type = "idle", body_part = 2 }
		data.unit:brain():action_request( new_action )
	elseif my_data.pathing_to_flee_pos then
		...
		if data.active_searches[ my_data.flee_path_search_id ] then
			...
			data.active_searches[ my_data.flee_path_search_id ] = nil
...
function CopLogicTrade.action_complete_clbk( data, action )
	...
	if action_type == "walk" then
		if my_data.walking_to_flee_pos then
