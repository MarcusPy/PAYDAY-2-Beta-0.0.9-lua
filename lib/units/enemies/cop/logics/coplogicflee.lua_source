...
function CopLogicFlee.update( data )
	...
	if data.attention_obj and data.attention_obj.reaction >= AIAttentionObject.REACT_COMBAT then
		...
		elseif my_data.cover_path then
			if not ( my_data.turning or unit:movement():chk_action_forbidden( "walk" ) ) then
...
function CopLogicFlee._update_enemy_detection( data )
	...
	--print( "[CopLogicFlee._update_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
...
function CopLogicFlee._upd_shoot( data, my_data )
	...
	if not action_taken then
		if my_data.advance_blocked and data.unit:anim_data().move then
			...
			if not CopLogicAttack._chk_request_action_turn_to_enemy( data, my_data, data.m_pos, focus_enemy.m_pos ) then
	...
	if data.pathing_results then
		...
		if path then
			...
			if path ~= "failed" then
				...
				--print( "CopLogicFlee:_update_pathing() we have an flee path" )
			else
		...
		if path then
			...
			if path ~= "failed" then
				...
				--print( "CopLogicFlee:_update_pathing() we have a coarse path" )
			else
				--print( "CopLogicFlee:_update_pathing() coarse_path failed!" )
	...
	if data.pathing_results then
		...
		if path then
			if path ~= "failed" then
				...
				--print( "CopLogicFlee:_update_cover_pathing() we have a cover path" )
			else
...
-----------------------------------------------------------------------------
-- Returns if its a good idea to attempt to arrest or assault this criminal
function CopLogicFlee._chk_reaction_to_attention_object( data, attention_data, stationary )
	...
	elseif not record.being_arrested then -- If he is being arrested, ignore
		local my_vec = data.m_pos - attention_data.m_pos
		local dis = mvector3.normalize( my_vec )
		if dis < 500 then -- If he is too close, assault
			return math.min( attention_data.settings.reaction, AIAttentionObject.REACT_COMBAT )
		elseif dis < 3000 then	-- If he is in close shooting range and pointing in my direction and has been recently aggressive, assault
			local criminal_fwd = att_unit:movement():m_head_rot():y()
			local criminal_look_dot = mvector3.dot( my_vec, criminal_fwd )
			if criminal_look_dot > 0.9 then	-- He is pointing in my direction
			...
			if record.engaged_force == 0 or record.engaged_force == 1 and record.engaged[ data.unit:key() ] then
				return math.min( attention_data.settings.reaction, AIAttentionObject.REACT_COMBAT )	-- I am isolated, need to cover my own back
			end
			local my_data = data.internal_data
			--[[if my_data.advancing then
				...
				if walk_to_pos then
					...
					if move_dot < -0.25 then
						my_data.advance_blocked = true
						return math.min( attention_data.settings.reaction, AIAttentionObject.REACT_COMBAT )
					end
				end
			else]]
...
function CopLogicFlee.action_complete_clbk( data, action )
	--print( "CopLogicFlee.action_complete_clbk", action:type() )
	...
	if action_type == "walk" then
		...
		else	-- we are moving towards our next flee pos
			if my_data.best_cover then	-- We are leaving our old cover
...
function CopLogicFlee._update_cover( data )
	--print( "CopLogicFlee._update_cover", data.t )
	local my_data = data.internal_data
	...
	if want_cover and data.attention_obj then
		...
		if not my_data.moving_to_cover then -- We are not allowed to change our best_cover when moving ( for administrative reasons )
			if not ( best_cover and CopLogicAttack._verify_cover( best_cover[1], threat_pos, min_threat_dis ) and mvector3.distance( best_cover[1][1], threat_pos ) > min_threat_dis ) then
				--print( "best_cover not verified" )
				local better_cover
				
				if nearest_cover and CopLogicAttack._verify_cover( nearest_cover[1], threat_pos, min_threat_dis ) then	-- Our previous cover will do
					better_cover = nearest_cover
					--print( "switching back to nearest cover" )
				end
				...
				if better_cover then
					if best_cover then
						...
						--print( "cancelling pathing to old best_cover" )
						CopLogicFlee._cancel_cover_pathing( data, my_data )
					end
					my_data.best_cover = better_cover
					managers.navigation:reserve_cover( better_cover[1], data.pos_rsrv_id )	-- If this is the nearest cover then we are reserving twice but it's not a bug
				end
			end
		end
	else
		if nearest_cover and mvector3.distance( nearest_cover[1][1], data.m_pos ) > cover_release_dis	then -- I dont want cover and have one
			managers.navigation:release_cover( nearest_cover[1] )
			my_data.nearest_cover = nil
		end
		if best_cover and mvector3.distance( best_cover[1][1], data.m_pos ) > cover_release_dis	then -- I dont want cover and have one
			managers.navigation:release_cover( best_cover[1] )
			my_data.best_cover = nil
		end
	end
