...
function CopLogicSniper.enter( data, new_logic_name, enter_params )
	...
	--print( "[CopLogicSniper.enter]", data.unit:key() )
	...
	if not my_data.rsrv_pos.stand then
		local pos_rsrv = { position = mvector3.copy( data.m_pos ), radius = 100, filter = data.pos_rsrv_id }
...
function CopLogicSniper.exit( data, new_logic_name, enter_params )
	--debug_pause_unit( data.unit, "[CopLogicSniper.exit]", data.unit )
...
function CopLogicSniper._upd_enemy_detection( data )
	...
	--print( "[CopLogicSniper._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
...
function CopLogicSniper.action_complete_clbk( data, action )
	--print( "[CopLogicSniper.action_complete_clbk]", action:type() )
	...
	elseif action_type == "hurt" and ( action:body_part() == 1 or action:body_part() == 2 ) then
		if data.objective and data.objective.pos then
...
function CopLogicSniper._upd_aim( data, my_data )
	...
	-- check full-body / lower-body actions
	...
	if not action_taken then
		...
		elseif my_data.attitude == "engage" and not data.is_suppressed then
			if focus_enemy then
				if not CopLogicAttack._chk_request_action_turn_to_enemy( data, my_data, data.m_pos, focus_enemy.verified_pos or focus_enemy.m_head_pos ) then
					if not ( focus_enemy.verified or anim_data.reload ) then
			...
			if focus_enemy then
				if not CopLogicAttack._chk_request_action_turn_to_enemy( data, my_data, data.m_pos, focus_enemy.verified_pos or focus_enemy.m_head_pos ) then
					if focus_enemy.verified then
						if anim_data.stand and data.char_tweak.allow_crouch then
							if CopLogicSniper._chk_crouch_visibility( data.m_pos, focus_enemy.m_head_pos, data.visibility_slotmask ) then
								CopLogicAttack._chk_request_action_crouch( data )
							end
						end
					end
	...
	if my_data.reposition and not ( action_taken or my_data.advancing ) then
		local objective = data.objective
		my_data.advance_path = { mvector3.copy( data.m_pos ), mvector3.copy( objective.pos ) }
