		...
		if not action_taken then
			...
			elseif my_data.optimal_pos and focus_enemy.nav_tracker then
				...
				if ray_res then
					...
					-- If ideal position is behind me
					if fwd_dot > 0 then
						local enemy_tracker = focus_enemy.nav_tracker
						if enemy_tracker:lost() then	-- do not raycast from lost trackers
				...
				local do_move = mvector3.distance_sq( to_pos, data.m_pos ) > 100*100
				if not do_move then
					--[[if tostring( data.key ) == debug_key then
						print( "optimal pos too close" )
					end]]
					...
					if fwd_bump_current then
						--[[if tostring( data.key ) == debug_key then
							print( "moving due to fwd_bump_current" )
						end]]
						do_move = true
						--Application:draw_cylinder( to_pos, to_pos + math.UP * 150, 20, 0.5,0.5,0.5 )
					end
				--else
					--Application:draw_cylinder( to_pos, to_pos + math.UP * 150, 20, 1,1,1 )
				end
--					Draw:brush(Color(0.3, 1,1,1), 0.5):sphere(to_pos, 25)
--					Draw:brush(Color(0.3, 0,0,0), 0.5):sphere(data.m_pos, 25)

--					local z_dist = math.abs(to_pos.z - data.m_pos.z)
--					if mvector3.distance(to_pos, data.m_pos) > 300 or z_dist > 200 then
				if do_move then
					...
					if reservation then
						...
						reservation = { position = mvector3.copy( to_pos ), radius = 70, filter = data.pos_rsrv_id }
...
function ShieldLogicAttack:_reserve_pos_step_clbk(data, test_pos)
	...
	-- Yes, this should mutate test_pos 
...
function ShieldLogicAttack._chk_request_action_walk_to_optimal_pos( data, my_data, end_rot )
	if not data.unit:movement():chk_action_forbidden( "walk" ) then
		...
		if my_data.walking_to_optimal_pos then
			...
			if data.group and data.group.leader_key == data.key then
				if data.char_tweak.chatter.follow_me and mvector3.distance( new_action_data.nav_path[ #new_action_data.nav_path ], data.m_pos ) > 800 and not data.unit:sound():speaking( data.t ) then
...
function ShieldLogicAttack._upd_enemy_detection( data )
	...
	local detected_enemies = data.detected_attention_objects	-- Enemies whose whereabouts I am personally aware of
	local enemies = {}
	local enemies_cpy = {}
	...
	for key, enemy_data in pairs( detected_enemies ) do
		-- Filter non-visible for 8sec
		if enemy_data.reaction >= AIAttentionObject.REACT_SCARED and enemy_data.identified and enemy_data.verified_t and enemy_data.verified_t > verified_chk_t then
	...
	if threat_epicenter then
		...
		--[[
		local line_brush = Draw:brush(Color(0.3, 0,1,0), 0.5)
		local point_brush = Draw:brush(Color(0.3, 0,0,1), 0.5)
		Draw:brush(Color(0.3, 1,0,0), 0.5):sphere(threat_epicenter, 25)
		]]
		...
		--if not my_data.threat_epicenter or mvector3.distance(threat_epicenter, my_data.threat_epicenter) >= 0 then
			...
			for key1, enemy_data1 in pairs(enemies) do
				...
				for key2, enemy_data2 in pairs(enemies_cpy) do
					...
					--line_brush:cylinder(enemy_data1.m_pos, enemy_data2.m_pos, 15)
					
					if mvector3.dot(from_threat, to_pt) > 0 then
						--point_brush:sphere(pt, 25)
...
--			point_brush:cylinder(furthest_line[1], furthest_line[2], 15)
		else
		...
		--point_brush:cylinder(my_data.optimal_pos, data.m_pos, 15)
		...
		for key, enemy_data in pairs( enemies ) do
			...
			if not focus_enemy_reaction or focus_enemy_reaction <= reaction then
				...
				if not focus_enemy or enemy_data.verified and not focus_enemy.verified or not ( not enemy_data.verified and focus_enemy.verified ) and focus_enemy_angle < angle then
	...
	else -- no recently visible enemies
		...
		--print( "[ShieldLogicAttack._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
...
function ShieldLogicAttack.action_complete_clbk( data, action )
	...
	if action_type == "walk" then
		...
		if action:expired() then
			...
			local reservation = { position = mvector3.copy( data.m_pos ), radius = 70, filter = data.pos_rsrv_id }
			managers.navigation:add_pos_reservation( reservation )
...
function ShieldLogicAttack.chk_wall_distance( data, my_data, pos, second_pass )
	...
	--[[if tostring( data.key ) == debug_key then
		print( "[ShieldLogicAttack.chk_wall_distance]", pos, second_pass )
	end]]
	local my_tracker = managers.navigation:create_nav_tracker( pos )
	local tracker_lost = my_tracker:lost()
	--[[if tracker_lost then
		debug_pause_unit( data.unit, "[ShieldLogicAttack.chk_wall_distance] tracker lost" )
	...
	--[[if tostring( data.key ) == debug_key then
		debug_pause_unit( data.unit, "[ShieldLogicAttack.chk_wall_distance] bump", col_side, "\n", inspect( ray_params ) )
		Application:draw_cylinder( pos, ray_params.pos_to, 25, 0.5,0,0.5 )
		Application:draw_cylinder( pos, col_pos, 20, 1,0,1 )
	end]]
	
	local correction_vec = ray_params.pos_to - col_pos
	mvector3.multiply( correction_vec, 1.05 ) -- +5% extra space
	...
	--[[if tostring( data.key ) == debug_key then
		Application:draw_cylinder( pos, walk_to_pos, 15, 1,0,0 )
	end]]
	...
	if managers.navigation:raycast( ray_params ) then
		...
		--[[if tostring( data.key ) == debug_key then
			print( "hit bwd too" )
			Application:draw_cylinder( pos, walk_to_pos, 12, 0.5,0,0 )
		end]]
	end
