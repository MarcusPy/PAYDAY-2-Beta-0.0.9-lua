...
function ObjectiveUnitElement:update_sub_objectives()
	...
	-- table.insert( sub_objectives, 1, "none" )
	self._hed.sub_objective = "none"
...
function ObjectiveUnitElement:_build_panel( panel, panel_sizer )
	...
	local state_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= { "activate", "complete", "update", "remove" },
	...
	sub_objective:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = sub_objective, value = "sub_objective" } )
