...
function EnemyPreferedAddUnitElement:update_selected( t, dt, selected_unit, all_units )
	local _draw_func = function( element_ids )
		...
		for _,id in ipairs( element_ids ) do
			...
			if draw then
				self:_draw_link( { from_unit = self._unit, to_unit = unit,  r = 0, g = 0.0, b = 0.75 } )
			end
		end
	end
...
function EnemyPreferedAddUnitElement:remove_links( unit )
	...
	local _rem_func = function( element_ids )
		if not element_ids then
			return
		end
...
function EnemyPreferedRemoveUnitElement:update_editing() -- Enables editing
end
...
function EnemyPreferedRemoveUnitElement:draw_links( t, dt, selected_unit, all_units )
	...
	for _,id in ipairs( self._hed.elements ) do
		...
		local draw = not selected_unit or ( unit == selected_unit ) or self._unit == selected_unit
		if draw then
			self:_draw_link( { from_unit = self._unit, to_unit = unit,  r = 0.75, g = 0.0, b = 0 } )
		end
	end
end
