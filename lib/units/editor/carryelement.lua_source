...
function CarryUnitElement:_build_panel( panel, panel_sizer )
	...
	--local opt_sizer = EWS:StaticBoxSizer( panel, "VERTICAL", "Filter" )
		...
		local operation_params = {
			...
			panel 				= panel,
			sizer 				= panel_sizer,
			options				= { "remove", "freeze", "secure", "secure_silent", "add_to_respawn" },
		...
		type_filter:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = type_filter, value = "type_filter" } )
end
