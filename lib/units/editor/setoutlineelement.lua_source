...
function SetOutlineElement:_build_panel( panel, panel_sizer )	
	...
	set_outline:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = set_outline, value = "set_outline" } )
...
function SetOutlineElement:update_selected( t, dt, selected_unit, all_units )
	for _,id in ipairs( self._hed.elements ) do
		...
		if draw then
			self:_draw_link( { from_unit = self._unit, to_unit = unit,  r = 0.9, g = 0.5, b = 1.0 } )
		end
	end
end
...
function SetOutlineElement:add_element()
	...
	if ray and ray.unit  
		and( string.find( ray.unit:name():s(), "ai_spawn_enemy", 1 ,true ) 
		or string.find( ray.unit:name():s(), "ai_spawn_civilian", 1 ,true )) then
