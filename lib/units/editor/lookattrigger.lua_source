...
function LookAtTriggerUnitElement:_build_panel( panel, panel_sizer )	
	...
	sensitivity_params.slider_ctrlr:connect( "EVT_SCROLL_THUMBTRACK", callback( self, self, "set_element_data" ),{ ctrlr = sensitivity_params.number_ctrlr, value = "sensitivity" } )
	sensitivity_params.slider_ctrlr:connect( "EVT_SCROLL_CHANGED", callback( self, self, "set_element_data" ), { ctrlr = sensitivity_params.number_ctrlr, value = "sensitivity" } )
	
	sensitivity_params.number_ctrlr:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = sensitivity_params.number_ctrlr, value = "sensitivity" } )
	sensitivity_params.number_ctrlr:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = sensitivity_params.number_ctrlr, value = "sensitivity" } )
	...
	local help = {}
	help.text = 'Interval defines how offen the check should be done. Sensitivity defines how precise the look angle must be. A sensitivity of 0.999 means that you need to look almost directly at it, 0.5 means that you will get the trigger somewhere at the edge of the screen (might be outside or inside). \n\nDistance(in meters) can be used as a filter to the trigger (0 means no distance filtering)'
	help.panel = panel
