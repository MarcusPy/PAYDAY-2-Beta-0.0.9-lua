...
function EnemyDummyTriggerUnitElement:draw_links( t, dt, selected_unit, all_units )
	...
	for _,id in ipairs( self._hed.elements ) do
		...
		if draw then
			self:_draw_link( { from_unit = unit, to_unit = self._unit,  r = 0, g = 0.75, b = 0 } )
		end
	end
end
...
function EnemyDummyTriggerUnitElement:add_element()
	...
	if ray and ray.unit and self:_correct_unit( ray.unit:name():s() ) then --(string.find( ray.unit:name():s(), "ai_spawn_enemy", 1 ,true ) or string.find( ray.unit:name():s(), "ai_enemy_group", 1 ,true ) )then
...
function EnemyDummyTriggerUnitElement:_correct_unit( u_name )
	local names = { "ai_spawn_enemy", "ai_enemy_group", "ai_spawn_civilian", "ai_spawn_civilian", "ai_civilian_group" }
...
function EnemyDummyTriggerUnitElement:_build_panel( panel, panel_sizer )	
	...
	local event_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= { "death", "spawn", "panic", "weapons_hot", "anim_act_01", "anim_act_02", "anim_act_03", "anim_act_04", "anim_act_05", "anim_act_06", "anim_act_07", "anim_act_08", "anim_act_09", "anim_act_10" },
