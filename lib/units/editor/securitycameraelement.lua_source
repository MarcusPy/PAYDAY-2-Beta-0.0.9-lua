...
function SecurityCameraUnitElement:init( unit )
	...
	--self._hed.ai_enabled = nil
	--self._hed.apply_settings = nil
...
function SecurityCameraUnitElement:_build_panel( panel, panel_sizer )
	...
	apply_settings:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = apply_settings, value = "apply_settings" } )
	...
	detection_range:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = detection_range, value = "detection_range" } )
	detection_range:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = detection_range, value = "detection_range" } )
	...
	suspicion_range:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = suspicion_range, value = "suspicion_range" } )
	suspicion_range:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = suspicion_range, value = "suspicion_range" } )
	...
	detection_delay_min:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = detection_delay_min, value = "detection_delay_min" } )
	detection_delay_min:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = detection_delay_min, value = "detection_delay_min" } )
	...
	detection_delay_max:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = detection_delay_max, value = "detection_delay_max" } )
	detection_delay_max:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = detection_delay_max, value = "detection_delay_max" } )
end
...
function SecurityCameraUnitElement:_find_camera_raycast()
	...
	if not ( ray.unit:base() and ray.unit:base().security_camera ) then
		return
	end
...
function SecurityCameraUnitElement:update_selected( t, dt, selected_unit, all_units )
	...
	if not ( not selected_unit or self._camera_u_data and self._camera_u_data.unit == selected_unit or self._unit == selected_unit ) then
		return
	end
	
	if self._camera_u_data then
		self:_draw_link( { from_unit = self._unit, to_unit = self._camera_u_data.unit, r = 0, g = 0.75, b = 0 } )
	end
end
...
function SecurityCameraUnitElement:_align_camera_unit()
	if self._hed.apply_settings then
		...
		local new_yaw_rot = Rotation( 180 + self._hed.yaw, self._camera_u_data.original_rot_yaw:pitch(), self._camera_u_data.original_rot_yaw:roll() ) -- +180 is due to the root having -y for fwd
