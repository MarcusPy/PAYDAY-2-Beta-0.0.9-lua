...
function AccessCameraUnitElement:layer_finished()
	...
	--[[for _,id in pairs( self._hed.camera_u_ids ) do
		local unit = managers.worlddefinition:get_unit_on_load( id, callback( self, self, "load_unit" ) )
		if unit then
			self._camera_units[ unit:unit_data().unit_id ] = unit
		end
	end]]
end
...
function AccessCameraUnitElement:update_selected( t, dt, selected_unit, all_units )
	...
	if alive( self._camera_unit ) then
		self:_draw_link( { from_unit = self._unit, to_unit = self._camera_unit, r = 0, g = 0.75, b = 0 } )
		Application:draw( self._camera_unit, 0, 0.75, 0 )
	elseif self._hed.camera_u_id then
		self._hed.camera_u_id = nil
	end
	
	--[[for _,id in ipairs( self._hed.camera_u_ids ) do
		local unit = self._camera_units[ id ]
		if alive( unit ) then
...
function AccessCameraUnitElement:add_camera_uid()
	...
	if unit then
		...
		if table.contains( self._hed.camera_u_ids, id ) then
			table.delete( self._hed.camera_u_ids, id )
			self._camera_units[ id ] = nil
		else
			table.insert( self._hed.camera_u_ids, id )
			self._camera_units[ id ] = unit
		end]]
	end
		
	--[[if ray and ray.unit then
		-- if string.find( ray.unit:name():s(), "point_access_camera", 1, true ) then
		if ray.unit:name() == Idstring( "units/dev_tools/mission_elements/point_access_camera/point_access_camera" )
		or ray.unit:name() == Idstring( "units/dev_tools/mission_elements/ai_security_camera/ai_security_camera" ) then
...
function AccessCameraUnitElement:_build_panel( panel, panel_sizer )
	...
	pitch_limit:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = pitch_limit, value = "pitch_limit" } )
	pitch_limit:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = pitch_limit, value = "pitch_limit" } )
...
function AccessCameraTriggerUnitElement:_build_panel( panel, panel_sizer )
	...
	local trigger_type_params = {
		...
		sizer 				= panel_sizer,
		default				= "none",
		options				= { "accessed", "destroyed", "alarm" },
