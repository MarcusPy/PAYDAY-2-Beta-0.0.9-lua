...
function LootBagUnitElement:init( unit )
	...
	--[[self._hed.spawn_velocity = 0
	self._hed.spawn_mass = 0]]
	...
	--[[table.insert( self._save_values, "spawn_velocity" )
	table.insert( self._save_values, "spawn_mass" )]]
...
function LootBagUnitElement:test_element()
	...
	-- unit:push( self._hed.spawn_mass, self._hed.spawn_dir * self._hed.spawn_velocity )
...
function LootBagUnitElement:update_editing( time, rel_time )
	...
	local ray = managers.editor:unit_by_raycast( { from = from, to = to, mask = managers.slot:get_mask( "statics_layer" ) } )
...
function LootBagUnitElement:_build_panel( panel, panel_sizer )
	...
	push_multiplier:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = push_multiplier, value = "push_multiplier" } )
	push_multiplier:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = push_multiplier, value = "push_multiplier" } )
	...
	from_respawn:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = from_respawn, value = "from_respawn" } )
	...
	--[[local velocity_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		value 				= self._hed.spawn_velocity,
		floats 				= 0,
	...
	velocity:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = velocity, value = "spawn_velocity" } )
	velocity:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = velocity, value = "spawn_velocity" } )
	...
	mass:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = mass, value = "spawn_mass" } )
	mass:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = mass, value = "spawn_mass" } )]]
end
...
function LootBagTriggerUnitElement:draw_links( t, dt, selected_unit, all_units )
	...
	for _,id in ipairs( self._hed.elements ) do
		...
		if draw then
			self:_draw_link( { from_unit = unit, to_unit = self._unit,  r = 0.85, g = 0.85, b = 0.25 } )
		end
	end
end
...
function LootBagTriggerUnitElement:add_element()
	...
	if ray and ray.unit then
		if ray.unit:name() == Idstring( "units/dev_tools/mission_elements/point_loot_bag/point_loot_bag" ) then
...
function LootBagTriggerUnitElement:_build_panel( panel, panel_sizer )
	...
	trigger_type:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = trigger_type, value = "trigger_type" } )
