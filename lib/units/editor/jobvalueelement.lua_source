...
function JobValueFilterUnitElement:_build_panel( panel, panel_sizer )
	...
	local check_type_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= { "equal", "less_than", "greater_than", "less_or_equal", "greater_or_equal", "has_key", "not_has_key" },
...
function ApplyJobValueUnitElement:draw_links( t, dt, selected_unit, all_units )
	...
	for _,id in ipairs( self._hed.elements ) do
		...
		if draw then
			self:_draw_link( { from_unit = self._unit, to_unit = unit, r = 0.85, g = 0.85, b = 0.25 } )
		end
	end
end
...
function ApplyJobValueUnitElement:add_element()
	...
	if ray and ray.unit then
		-- if ray.unit:name() == Idstring( "core/units/mission_elements/logic_counter/logic_counter" ) then
