...
function PlayerStateUnitElement:init( unit )	
	...
	-- table.insert( self._save_values, "rule" )
end
...
function PlayerStateUnitElement:_build_panel( panel, panel_sizer )
	...
	local states_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= mixin_add( managers.player:player_states(), { "electrocution" } ),
	...
	--[[local rules_params = {
		...
		sizer 				= panel_sizer,
		options				= { "none", "no_run" },
		value 				= self._hed.rule,
		tooltip 			= "Select a rule from the combobox",
		name_proportions 	= 1,
		ctrlr_proportions 	= 2,
		sorted				= true
	}
	local rules = CoreEWS.combobox( rules_params )
	
	rules:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = rules, value = "rule" } )]]
		
	local help = {}
	help.text = 'Set the state the players should change to.'-- If a rule is set, the value in state will be ignored. The rule is a boolean value that will be toggled each time it is called.'
	help.panel = panel
