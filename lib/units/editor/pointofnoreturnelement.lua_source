...
function PointOfNoReturnElement:_build_panel( panel, panel_sizer )	
	...
	time_normal:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = time_normal, value = "time_normal" } )
	time_normal:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = time_normal, value = "time_normal" } )
	...
	time_overkill:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = time_overkill, value = "time_overkill" } )
	time_overkill:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = time_overkill, value = "time_overkill" } )
	...
	time_overkill_145:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = time_overkill_145, value = "time_overkill_145" } )
	time_overkill_145:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = time_overkill_145, value = "time_overkill_145" } )
...
function PointOfNoReturnElement:update_selected( t, dt, selected_unit, all_units )
	for _,id in ipairs( self._hed.elements ) do
		...
		if draw then
			self:_draw_link( { from_unit = self._unit, to_unit = unit,  r = 0.75, g = 0.0, b = 0.75 } )
		end
	end
end
