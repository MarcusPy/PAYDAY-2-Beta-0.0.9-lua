...
SpecialObjectiveUnitElement._AI_SO_types = { "AI_defend", "AI_security", "AI_hunt", "AI_search", "AI_idle", "AI_escort", "AI_sniper" }
...
function SpecialObjectiveUnitElement:init( unit )
	...
	self._hed.path_haste = "none" -- walk, run
	self._hed.path_stance = "none" -- crouch, stand
	self._hed.pose = "none" -- crouch, stand
	...
	self._hed.interrupt_dis = 7 -- float (meters) 0-#INF. interrupted if a threat is detected closer that this distance. -1 means at any distance
	self._hed.interrupt_dmg = 0	-- float 0-1. interrupted if total damage sustained exceeds this percentage of total health
...
function SpecialObjectiveUnitElement:post_init()
	if self._hed.follow_up_id then
		self._hed.followup_elements = { self._hed.follow_up_id }
	end
...
function SpecialObjectiveUnitElement:test_element()
	...
	local t = { id = self._unit:unit_data().unit_id, editor_name = self._unit:unit_data().name_id }
...
function SpecialObjectiveUnitElement:_draw_follow_up( selected_unit, all_units )
	if self._hed.followup_elements then
		for _, element_id in ipairs( self._hed.followup_elements ) do
			...
			local draw = not selected_unit or ( unit == selected_unit ) or self._unit == selected_unit
			if draw then
				self:_draw_link( { from_unit = self._unit, to_unit = unit,  r = 0, g = 0.75, b = 0 } )
			end
		end
	end
end
...
function SpecialObjectiveUnitElement:_lmb()
	...
	-- Else we set the new search position
...
function SpecialObjectiveUnitElement:selected()
	...
	if not managers.ai_data:patrol_path( self._hed.patrol_path ) then -- Path has been removed, default back to none
		self._hed.patrol_path = "none"
	end
	...
	if value then
		for i, k in ipairs(  self._nav_link_filter ) do
			if k == params.name then
				return -- we already have this flag
			end
		end
...
function SpecialObjectiveUnitElement:_build_panel( panel, panel_sizer )
	...
	local opt_sizer = EWS:StaticBoxSizer( panel, "VERTICAL", "Filter" )
		...
		local filter_preset_params = {
			...
			panel 				= panel,
			sizer 				= opt_sizer,
			options				= { "clear", "all" }, --, "enemies", "friendlies", "civilians", "bank_manager_old_man", "escort_guy_1", "escort_guy_2", "escort_guy_3", "escort_guy_4", "escort_guy_5", "chavez" },
	...
	local ai_group_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= { "enemies", "friendlies", "civilians", "bank_manager_old_man", "escort_guy_1", "escort_guy_2", "escort_guy_3", "escort_guy_4", "escort_guy_5", "chavez" },
	...
	is_navigation_link:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = is_navigation_link, value = "is_navigation_link" } )
	...
	align_rotation:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = align_rotation, value = "align_rotation" } )
	...
	align_position:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = align_position, value = "align_position" } )
	...
	needs_pos_rsrv:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = needs_pos_rsrv, value = "needs_pos_rsrv" } )
	...
	allow_followup_self:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = allow_followup_self, value = "allow_followup_self" } )
	...
	search_distance:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = search_distance, value = "search_distance" } )
	search_distance:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = search_distance, value = "search_distance" } )
	...
	for _, val in ipairs( self._AI_SO_types ) do -- add the AI SO to the list
	...
	patrol_path:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = patrol_path, value = "patrol_path" } )
	...
	path_stance:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = path_stance, value = "path_stance" } )
...
--		sorted				= true
	...
	local interaction_voice_params = {
		...
		panel 				= panel,
		sizer 				= panel_sizer,
		options				= { "default", "cuff_cop", "down_cop", "stop_cop", "escort_keep", "escort_go", "escort", "stop", "down_stay", "down", "bridge_codeword", "bridge_chair", "undercover_interrogate" },
...
--		sorted				= true
	...
	interaction_voice:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = interaction_voice, value = "interaction_voice" } )
	...
	interrupt_dis:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = interrupt_dis, value = "interrupt_dis" } )
	interrupt_dis:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = interrupt_dis, value = "interrupt_dis" } )
	...
	interrupt_dmg:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = interrupt_dmg, value = "interrupt_dmg" } )
	interrupt_dmg:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = interrupt_dmg, value = "interrupt_dmg" } )
	...
	action_duration_min:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = action_duration_min, value = "action_duration_min" } )
	action_duration_min:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = action_duration_min, value = "action_duration_min" } )
	...
	action_duration_max:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = action_duration_max, value = "action_duration_max" } )
	action_duration_max:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = action_duration_max, value = "action_duration_max" } )
