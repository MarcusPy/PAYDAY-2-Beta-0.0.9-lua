...
function ExplosionUnitElement:init( unit )	
	...
	self._hed.explosion_effect = "effects/particles/explosions/explosion_grenade_launcher" -- "none"
...
function ExplosionUnitElement:_build_panel( panel, panel_sizer )
	...
	player_damage:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = player_damage, value = "player_damage" } )
	player_damage:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = player_damage, value = "player_damage" } )
	...
	explosion_effect:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = explosion_effect, value = "explosion_effect" } )
...
function ExplosionUnitElement:add_to_mission_package()
	...
	if self._hed.explosion_effect ~= "none" then
		managers.editor:add_to_world_package( { category = "effects", name = self._hed.explosion_effect, continent = self._unit:unit_data().continent } )
	end
end
