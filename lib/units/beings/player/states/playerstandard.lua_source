...
function PlayerStandard:_get_input( t, dt )
	...
	if self._state_data.controller_enabled ~= self._controller:enabled() then
		-- This checks if the controller has been disabled this frame
...
function PlayerStandard:_stance_entered( unequipped )
	...
	managers.menu:set_mouse_sensitivity( new_fov < (misc_attribs.FOV or 75) ) -- self._state_data.in_steelsight and stances.steelsight.zoom_fov )
end
