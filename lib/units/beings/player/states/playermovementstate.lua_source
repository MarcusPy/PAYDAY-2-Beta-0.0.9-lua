...
--	Check if the current active action can be interrupted by the action type specified as a parameter
--	Action blocks are time based. a block time of -1 means permanently blocked
function PlayerMovementState:chk_action_forbidden( action_type )
	if self._current_action then
		...
		if unblock_data and ( unblock_data == -1 or unblock_data > managers.player:player_timer():time() ) then
			return true	--	Our current action forbids another action
		end
	end
end
...
function PlayerMovementState:_reset_current_action()
	...
	if previous_action then
		if self[ "_end_action_"..previous_action.type ] then
