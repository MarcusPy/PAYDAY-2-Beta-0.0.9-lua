...
function PlayerDamage:update( unit, t, dt )
	...
	elseif self._hurt_value then
		if not self._bleed_out then
			...
			managers.hud:set_player_armor( { current = self._armor * armor_value, total = self:_total_armor(), max = self:_max_armor() } )
...
function PlayerDamage:recover_health()
	...
	managers.hud:set_player_health( { current = self._health, total = self:_max_health(), revives = self._revives } )
end
...
function PlayerDamage:_regenerated( no_messiah )
	...
	self._revives = tweak_data.player.damage.LIVES_INIT + managers.player:upgrade_value( "player", "additional_lives", 0 ) 			-- when these are gone, you're out!
...
function PlayerDamage:set_health( health )
	...
	self._health = math.clamp( health, 0, max_health )  -- math.max( 0, health )  --
	self:_send_set_health()
	self:_set_health_effect()
	
	managers.hud:set_player_health( { current = self._health, total = max_health, revives = self._revives } )
end
...
function PlayerDamage:_calc_armor_damage( attack_data )
	...
	if self._armor > 0 then
		...
		self:set_armor( self._armor - attack_data.damage ) -- self._armor = math.max( 0, self._armor - attack_data.damage )
...
function PlayerDamage:_calc_health_damage( attack_data )
	...
	if( self._health == 0 and attack_data.variant and attack_data.variant == "bullet" and self._revives > 1 ) then			-- a bullet killed me, or did it?
		--[[
...
function PlayerDamage:damage_fall( data )
	...
	if die then
		...
		self:set_armor( self._armor - max_armor * managers.player:upgrade_value( "player", "fall_damage_multiplier", 1 ) ) -- self._armor = math.clamp( self._armor - max_armor * managers.player:upgrade_value( "player", "fall_damage_multiplier", 1 ), 0, max_armor )
	end
	managers.hud:set_player_armor( { current = self._armor, total = self:_total_armor(), max = max_armor, no_hint = true } )
...
function PlayerDamage:_check_bleed_out()
	if self._health == 0 then
		...
		self:on_downed()
	elseif not self._said_hurt and self._health/self:_max_health() < 0.2 then
...
function PlayerDamage:revive( helped_self )	-- This is called both for freeing and reviving. Separate the 2 in the future maybe
	...
	if not arrested then
		...
		self._health = math.clamp( self._health * tweak_data.player.damage.REVIVE_HEALTH_STEPS[ self._revive_health_i ] * ( self._revive_health_multiplier or 1 ), 0, self._health ) 
				-- math.round( tweak_data.player.damage.REVIVE_HEALTH_STEPS[ self._revive_health_i ] * self:_max_health() )
		self:set_armor( self:_total_armor() )  -- self._armor = self:_total_armor()
...
function PlayerDamage:_upd_health_regen( t, dt )	-- in percent of max health
	...
	if( not self._health_regen_update_timer ) then
		...
		if( regen_rate > 0 and self._health < max_health ) then
