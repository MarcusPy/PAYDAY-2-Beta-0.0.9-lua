...
function ShotgunBase:_fire_raycast( user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul ) -- ( user_unit, from_pos, direction )
	local result = {}	
	local hit_enemies = {}
	local hit_something
	local col_rays
...
function ShotgunBase:start_reload( ... )
	...
	self._next_shell_reloded_t = Application:time() + (20/30 - 0.33)/speed_multiplier
end
...
function ShotgunBase:update_reloading( t, dt, time_left )
	if self._next_shell_reloded_t < t then
		local speed_multiplier = self:reload_speed_multiplier()
		self._next_shell_reloded_t = self._next_shell_reloded_t + (20/30)/speed_multiplier
