...
function SentryGunWeapon:init( unit )
	...
	self._name_id = self.name_id	-- This is specified in the unit file
	self.name_id = nil	-- ??
	...
	self._interleaving_fire = 1 -- 1 means left, 2 means right
	
	self._muzzle_effect = Idstring( my_tweak_data.muzzleflash or "effects/particles/test/muzzleflash_maingun" )
	self._muzzle_effect_table = {
		{ effect = self._muzzle_effect, parent = self._effect_align[1], force_synch = false },
		{ effect = self._muzzle_effect, parent = self._effect_align[2], force_synch = false }
	}
	...
	self._trail_effect_table = { effect = RaycastWeaponBase.TRAIL_EFFECT, position = Vector3(), normal = Vector3() }
	
	self._ammo_sync_resolution = 0.2
	if Network:is_server() then
		self._ammo_total = 1	-- this get filled later on by set_ammo or sync_ammo
		self._ammo_max = self._ammo_total
		self._ammo_sync = 1	-- 0-1 last synched ammo percentage
	else
		self._ammo_ratio = 1 -- 0-1 percentage full in ammo
	end
	self._spread_mul = 1
end
...
function SentryGunWeapon:set_ammo( amount )	-- host only
...
function SentryGunWeapon:sync_ammo( ammo_ratio )	-- info sent from host. clientside only
...
function SentryGunWeapon:stop_autofire()
	...
	if self:out_of_ammo() then
		...
		if self._timer:time() - self._fire_start_t > 3 then
...
function SentryGunWeapon:load( save_data )
	if not ( save_data and save_data.weapon ) then
		self._spread_mul = 1
		return
	end
