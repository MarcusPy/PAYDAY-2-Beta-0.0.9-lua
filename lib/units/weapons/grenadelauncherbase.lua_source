...
function GrenadeLauncherBase:_fire_raycast( user_unit, from_pos, direction )
	...
	--[[local right = from_pos:cross( math.UP )
	mvector3.set_length(right, 6.2)
	mvector3.add(from_pos, right)
	mvector3.add(from_pos, math.UP * -13.2)]]
	
	local unit = M79GrenadeBase.spawn( "units/weapons/m79/grenade", from_pos, Rotation( direction, math.UP ) )
	unit:base():launch( { dir = direction, owner = self._unit, user = user_unit, damage = self._damage, range = range, curve_pow = curve_pow, alert_filter = user_unit:movement():SO_access() } )
