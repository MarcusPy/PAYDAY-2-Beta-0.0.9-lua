...
function CivilianLogicIdle.enter( data, new_logic_name, enter_params )
	...
	--print( "[CivilianLogicIdle.enter]", data.unit )
	...
	if not my_data.rsrv_pos.stand then
		local pos_rsrv = { position = mvector3.copy( data.m_pos ), radius = 45, filter = data.pos_rsrv_id }
	...
	if objective then
		if objective.action then
			if data.unit:brain():action_request( objective.action ) and objective.action.type == "act" then
				my_data.acting = true
				if objective.action_start_clbk then
					objective.action_start_clbk( data.unit )
					
					if my_data ~= data.internal_data then
						return -- we changed logic state in action_start_clbk
					end
				end
			end
		end
...
function CivilianLogicIdle.exit( data, new_logic_name, enter_params )
	--print( "[CivilianLogicIdle.exit]", data.unit, new_logic_name )
	CopLogicBase.exit( data, new_logic_name, enter_params )
	
	local my_data = data.internal_data
	
	if my_data.idle_attention then
		if alive( my_data.idle_attention.unit ) then
			CopLogicBase._reset_attention( data )
		end
	end
...
function CivilianLogicIdle._upd_outline_detection( data )
	...
	for e_key, record in pairs( managers.groupai:state():all_criminals() ) do
		if chk_vis_func( my_tracker, record.tracker ) then
			...
			if mvector3.distance_sq( enemy_pos, my_pos ) < 1200 * 1200 then
	...
	if seen then
		...
		-- Let seeing_unit say something here?
	else
...
function CivilianLogicIdle.on_alert( data, alert_data )
	--debug_pause_unit( data.unit, "[CivilianLogicIdle.on_alert]", data.unit, inspect( alert_data ) )
	local my_data = data.internal_data
	...
	if not my_data.delayed_alert_id then
		...
		CopLogicBase.add_delayed_clbk( my_data, my_data.delayed_alert_id, callback( CivilianLogicIdle, CivilianLogicIdle, "_delayed_alert_clbk", { data = data, alert_data = clone( alert_data ) } ), TimerManager:game():time() + alert_delay )
	end
end
...
function CivilianLogicIdle._delayed_alert_clbk( ignore_this, params )
	...
	if not CivilianLogicIdle.is_obstructed( data, alerting_unit ) then -- keep trying to get the alert
		my_data.delayed_alert_id = "alert"..tostring( data.key )
	...
	alert_data[5] = alerting_unit -- alert_data[5] must not be destroyed unit
...
function CivilianLogicIdle.on_intimidated( data, amount, aggressor_unit )
	...
	data.unit:brain():set_objective( { type = "surrender", amount = amount, aggressor_unit = aggressor_unit } )
end
...
function CivilianLogicIdle.damage_clbk( data, damage_info )
	...
	data.unit:brain():set_objective( { type = "free", is_default = true, dmg_info = damage_info } )
end
...
function CivilianLogicIdle.on_new_objective( data, old_objective )
	...
	--print( "[CivilianLogicIdle.on_new_objective] old_objective", inspect( old_objective ), "new_objective", inspect( new_objective ), data.unit:movement():cool() )
	local my_data = data.internal_data
...
function CivilianLogicIdle.action_complete_clbk( data, action )
	...
	elseif action:type() == "act" then
		if my_data.acting and data.objective then
			my_data.acting = nil
...
function CivilianLogicIdle._upd_detection( data )
	...
	if not data.unit:movement():cool() and not ( my_data.acting and not data.unit:anim_data().act_idle ) then
		local objective = data.objective
		if not objective or objective.interrupt_dis == -1 or objective.is_default then
			local alert = { "vo_cbt", data.m_pos, nil, nil, nil }
			CivilianLogicIdle.on_alert( data, alert )
			if my_data ~= data.internal_data then
				return
			end
...
function CivilianLogicIdle.is_available_for_assignment( data, objective )
	...
	local my_data = data.internal_data
	return not ( ( my_data.acting and not data.unit:anim_data().act_idle ) or my_data.exiting or my_data.delayed_alert_id )
end

-----------------------------------------------------------------------------

function CivilianLogicIdle.anim_clbk( data, info_type )
	if info_type == "reset_attention" then
		if data.internal_data.idle_attention then
		...
		elseif attention_data.pause_expire_t then
			if data.t > attention_data.pause_expire_t then
				if not attention_data.settings.attract_chance or math.random() < attention_data.settings.attract_chance then
					...
					--debug_pause_unit( data.unit, "[ CivilianLogicIdle._get_priority_attention] skipping attraction for", attention_data.pause_expire_t - data.t, "sec" )
				end
			end
