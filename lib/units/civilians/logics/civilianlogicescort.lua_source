...
function CivilianLogicEscort.enter( data, new_logic_name, enter_params )
	...
	--print( "[CivilianLogicEscort.enter]" )
...
function CivilianLogicEscort.update( data )
	...
	if CivilianLogicEscort.too_scared_to_move( data ) then
		if not data.unit:anim_data().panic then
			my_data.commanded_to_move = nil
			data.unit:movement():action_request( { type = "act", body_part = 1, variant = "panic", clamp_to_graph = true } )
		end
	end
...
function CivilianLogicEscort.on_intimidated( data, amount, aggressor_unit )
	...
	if scared_reason then
		-- Say something here about why he doesn't want to move
	...
	if path_data then
		...
		elseif path_style == "coarse" then
			...
			while i_point <= #path_data.points do
				...
				t_ins( coarse_path, { next_seg, mvector3.copy( next_pos ) } )
				i_point = i_point + 1
			end
...
function CivilianLogicEscort.too_scared_to_move( data )
	...
	local min_dis_sq = 1000*1000 -- 10m
	...
	if nobody_close then
		return "abandoned"	-- There are no criminals around us
	end
	...
	for c_key, c_data in pairs( managers.enemy:all_enemies() ) do
		if not ( c_data.unit:anim_data().surrender or c_data.unit:brain()._current_logic_name == "trade" ) and mvector3.distance_sq( c_data.m_pos, data.m_pos ) < min_dis_sq and math.abs( ( c_data.m_pos.z - data.m_pos.z ) ) < 250 then
			nobody_close = nil
			break
		end
	end
	if not nobody_close then
		return "pigs"	-- There are pigs near!
	end
