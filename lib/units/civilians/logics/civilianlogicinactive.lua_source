...
function CivilianLogicInactive._register_attention( data, my_data )
	if data.unit:character_damage():dead() and not managers.groupai:state():enemy_weapons_hot() then
		...
		managers.groupai:state():add_listener( my_data.weapons_hot_listener_key, { "enemy_weapons_hot" }, callback( CivilianLogicInactive, CivilianLogicInactive, "on_enemy_weapons_hot", data ) )
