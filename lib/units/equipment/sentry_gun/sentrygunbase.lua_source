...
function SentryGunBase.spawn( owner, pos, rot, ammo_multiplier, armor_multiplier, damage_multiplier )
	...
	unit:base():setup( owner, ammo_multiplier, armor_multiplier, damage_multiplier, spread_multiplier, rot_speed_multiplier, has_shield, attached_data ) -- unit removes itself if this call fails
...
function SentryGunBase:setup( owner, ammo_multiplier, armor_multiplier, damage_multiplier, spread_multiplier, rot_speed_multiplier, has_shield, attached_data )
	...
	local ammo_amount = tweak_data.upgrades.sentry_gun_base_ammo * ammo_multiplier  -- + managers.player:upgrade_value_by_level( "sentry_gun", "ammo_increase", ammo_upgrade_lvl )
	self._unit:weapon():set_ammo( ammo_amount )
	local armor_amount = tweak_data.upgrades.sentry_gun_base_armor * armor_multiplier -- + managers.player:upgrade_value_by_level( "sentry_gun", "armor_increase", armor_upgrade_lvl )
	...
	self._unit:brain():setup( 1 / rot_speed_multiplier ) -- rot_speed_multiplier is used here as a "sharpness multiplier". reduces the angle to which the sentrygun starts firing, thus wasting wasting less ammo
	
	local setup_data = {}
	setup_data.user_unit = self._owner
	setup_data.ignore_units = { self._unit, self._owner }
...
function SentryGunBase:_check_body()
	...
	elseif self._attached_data.index == 2 then
		if not ( alive( self._attached_data.body ) and mrotation.equal( self._attached_data.rotation, self._attached_data.body:rotation() ) ) then
			self._attached_data = self._attach( nil, nil, self._unit )
			if not self._attached_data then
				self:remove()
				return
