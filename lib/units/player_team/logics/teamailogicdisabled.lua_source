...
function TeamAILogicDisabled.enter( data, new_logic_name, enter_params )
	...
	--print( "[TeamAILogicDisabled.enter]" )
	...
	--TeamAILogicDisabled._consider_surrender( data, my_data )
...
function TeamAILogicDisabled.exit( data, new_logic_name, enter_params )
	...
	--print( "[TeamAILogicDisabled.exit]" )
...
function TeamAILogicDisabled._upd_enemy_detection( data )
	...
	--print( "[TeamAILogicDisabled._upd_enemy_detection] new_attention", inspect( new_attention ), "settings", new_attention and inspect( new_attention.settings ), "new_reaction", new_reaction )
...
function TeamAILogicDisabled.on_intimidated( data, amount, aggressor_unit )
	--[[local my_data = data.internal_data
	if data.t > my_data.stay_cool_chk_t + 7 then
		TeamAILogicDisabled._consider_surrender( data, my_data )
	end]]
end
...
function TeamAILogicDisabled._consider_surrender( data, my_data )
	...
	for c_key, c_data in pairs( managers.groupai:state():all_player_criminals() ) do
		if not c_data.status then	-- not downed or arrested
...
function TeamAILogicDisabled._upd_aim( data, my_data )
	...
	if my_data.stay_cool then
		-- just don't
	elseif focus_enemy then	-- I have an enemy
...
function TeamAILogicDisabled.on_recovered( data, reviving_unit )
	local my_data = data.internal_data
	if reviving_unit and my_data.rescuer and my_data.rescuer:key() == reviving_unit:key() then -- the reviving unit is an AI
		my_data.rescuer = nil
	else	-- the reviving unit is a human player
		TeamAILogicDisabled._unregister_revive_SO( my_data )
	end
...
function TeamAILogicDisabled.damage_clbk( data, damage_info )
	...
	if data.unit:character_damage():need_revive() and not ( my_data.SO_id or my_data.rescuer ) then
		TeamAILogicDisabled._register_revive_SO( data, my_data, "revive" )
	end
...
function TeamAILogicDisabled.on_revive_SO_failed( ignore_this, data )
	local my_data = data.internal_data
	if my_data.rescuer and ( data.unit:character_damage():need_revive() or data.unit:character_damage():arrested() ) and not my_data.exiting then-- this callback could be due to the fact that we died
		my_data.rescuer = nil	-- this teammate failed to rescue us. re-register the SO immediately
		TeamAILogicDisabled._register_revive_SO( data, my_data, "revive" )
	end
end
