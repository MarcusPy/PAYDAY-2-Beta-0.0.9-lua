...
function SpecialEquipmentPickup:_pickup( unit )
	...
	if not unit:character_damage():dead() and equipment then
		if managers.player:can_pickup_equipment( self._special ) then
			managers.player:add_special( { name = self._special, amount = self._amount } )
