...
function MissionDoor:activate()
	...
	for type,device_data in pairs( devices_data ) do
		...
		self._devices[ type ] = { units = {}, placed = false, completed = false, amount = amount, placed_counter = 0, completed_counter = 0 }
		for _,unit_data in ipairs( device_data ) do
			...
			-- self._devices[ type ] = { unit = unit, placed = false, completed = false }
			table.insert( self._devices[ type ].units, { unit = unit, placed = false, completed = false } )
		end
	end
end
	...
	if self._devices[ type ].completed_counter == self._devices[ type ].amount then
		...
		elseif type == "c4" then
			...
			if Network:is_server() then
				...
				local alert_event = { "aggression", self._unit:position(), tweak_data.weapon.trip_mines.alert_radius, managers.groupai:state():get_unit_type_filter("civilians_enemies"), self._unit }
...
function MissionDoor:_initiate_c4_sequence()
	...
	for type,device in pairs( self._devices ) do
		if type ~= "c4" then
			...
			--[[if alive( device.unit ) then
				device.unit:set_slot( 0 )
			end]]
		end
	end
...
--[[ Run sequences on devices through door (jam drill, power drill etc )
function MissionDoor:run_device_sequence( type, trigger_sequence )
	if alive( self._devices[ type ].unit ) then
		self._devices[ type ].unit:run_sequence_simple( trigger_sequence )
	end
end
]]
...
function MissionDoorDevice:init( unit )
	self._unit = unit
	-- MissionDoorDevice.super.init( self, unit, false )
end
