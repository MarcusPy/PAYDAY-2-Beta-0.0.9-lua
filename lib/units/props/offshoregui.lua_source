...
function OffshoreGui:setup()
	...
	self._ws:panel():rect( { color = Color.black, layer = -1 } )
	...
	local text = managers.localization:to_upper_text( "menu_offshore_account" ) -- "CAYMAN ISLAND ACCOUNT" 
	self._title_text = panel:text( { text = text, y = -self._ws:panel():h()/2 - default_offset, font = "fonts/font_medium_noshadow_mf", align = "center", vertical = "bottom", font_size = font_size, layer = 0, visible = true, color = OffshoreGui.TITLE_COLOR } )
	
	local font_size = 220
	local money_text = managers.experience:cash_string( managers.money:offshore() )
	self._title_text = panel:text( { text = money_text, y = self._ws:panel():h()/2 - default_offset, font = "fonts/font_medium_noshadow_mf", align = "center", vertical = "top", font_size = font_size, layer = 0, visible = true, color = OffshoreGui.MONEY_COLOR } )
	-- self._ws:panel():video( { layer = 10, visible = true, video = self._video, loop = true } )
	-- self._title = self._ws:panel():text( { layer = 10, visible = true, text = , loop = true } )
end
