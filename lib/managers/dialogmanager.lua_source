	...
	if not self._pause and self._current_dialog then
		...
		if self._current_node_timer <= 0 then
			...
			if not self._queued_dialog then
				-- Play the next action in the node

				node_data:update( self._current_dialog_params )
			else
				-- Play the queued dialog and stop the current one
...
--[[ params can contain:
	case				- If the dialog uses a case node, it will be checked with this value
	override_characters	- Used for testing, will play the entire dialog from the player unit (ignoring that the characters isnt available)
	on_unit				- Used for testing, if provided the dialog will be played on the unit
]]
function DialogManager:queue_dialog( id, params )
	-- This check will prevent dialogs(BAIN) from being played in load out.
	...
	if not self._current_dialog then
		...
		if self._queued_dialog then
			if dialog._priority > self._queued_dialog._priority then
				return
			end
		end
...
function DialogManager:_load_dialog_data( name )
	...
	for _,c in ipairs( data ) do
		if( c._meta == "node_definitions" ) then
			-- Save the node definitions in a list

			for _,node in ipairs( c ) do
				if( node.id and node.character and node.drama_cue ) then
					self._node_definition_list[ node.id ] = { character = node.character, drama_cue = node.drama_cue }
				else
...
function DialogAction:check_variable( variable )
	if not variable.name then
		-- If there is no variable check, always run the action
		return true
	else
		...
		elseif variable.condition == "not_equal" then
			if variable.value ~= value then
				return true
			end
		end
...
function DialogActionLine:execute( params )
	if DialogAction:check_variable( self._variable ) then
		-- self._unit = managers.dialog:_unit( self._node.character )
		-- self._unit = managers.player:player_unit()
		self._unit = params.on_unit or params.override_characters and managers.player:player_unit()
		if not alive( self._unit ) then 		
			if self._node.character == "dispatch" then -- Play dispatch on player unit
		...
		-- 	Application:throw_exception( "The dialog script tries to access a unit named '" .. tostring( self._node.character ) .. "', which doesn't seem to exist!" ) 
		-- end
		
		if alive( self._unit ) then
			--print( self._unit, self._unit:drama() )
			--print( self._node )
...
function DialogActionVariable:execute( ... )
	...
	-- Check if value should be combine the old value
...
function DialogActionCallback:init( data_node )
	...
	if self._class then
		...
		if not DialogCallbacks[ self._callback ] then
...
function DialogActionOption:init( data_node )
	...
	self._data = { string_id = data_node.string_id, go_to_node = data_node.go_to_node }
	self._variable = DialogAction:setup_variable( data_node )
end
