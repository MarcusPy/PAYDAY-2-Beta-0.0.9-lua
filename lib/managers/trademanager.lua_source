	...
	if not self._hostage_remind_t or t > self._hostage_remind_t then
		if not self._trading_hostage and 
			...
			if managers.groupai:state():bain_state() then
				...
				if cable_tie_data and cable_tie_data.amount > 0 then
	...
	if self._trade_countdown then
		if 	Network:is_server() and
			not self._trading_hostage and 
		   	not self._hostage_trade_clbk and
		   	#self._criminals_to_respawn > 0 then
		   	
		   	if self:get_criminal_to_trade() ~= nil then
...
function TradeManager:on_player_criminal_death( criminal_name, respawn_penalty, hostages_killed, skip_netsend )
	...
	for i, crim_to_respawn in ipairs(self._criminals_to_respawn) do
		if crim_to_respawn.ai == true or crim_to_respawn.respawn_penalty > respawn_penalty then
...
function TradeManager:replace_ai_with_player( ai_criminal, player_criminal, new_respawn_penalty )
	...
	if Network:is_server() then
		managers.network:session():send_to_peers( "set_trade_replace", true, ai_criminal, player_criminal, respawn_penalty )
	end
	]]
end
...
function TradeManager:replace_player_with_ai( player_criminal, ai_criminal, new_respawn_penalty )
	...
	--[[
	if Network:is_server() then
		managers.network:session():send_to_peers( "set_trade_replace", false, player_criminal, ai_criminal, respawn_penalty )
	end
	]]
end
