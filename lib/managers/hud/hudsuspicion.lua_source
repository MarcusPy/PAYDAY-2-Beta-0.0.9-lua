...
function HUDSuspicion:init( hud, sound_source )
	...
	local scale = 1.175
	local suspicion_left = self._suspicion_panel:bitmap( { name = "suspicion_left", visible = true, texture = "guis/textures/pd2/hud_stealthmeter", color = Color(0,1,1), alpha = 1, valign = "center", w = 128, h = 128, blend_mode="add", render_template = "VertexColorTexturedRadial", layer=1 } )
	...
	local suspicion_right = self._suspicion_panel:bitmap( { name = "suspicion_right", visible = true, texture = "guis/textures/pd2/hud_stealthmeter", color = Color(0,1,1), alpha = 1, valign = "center", w = 128, h = 128, blend_mode="add", render_template = "VertexColorTexturedRadial", layer=1 } )
	...
	local hud_stealthmeter_bg = self._misc_panel:bitmap( { name = "hud_stealthmeter_bg", visible = true, texture = "guis/textures/pd2/hud_stealthmeter_bg", color = Color(0.2,1,1,1), alpha = 0, valign = {1/2,0}, w = 128, h = 128, blend_mode="normal" } )
	...
	local suspicion_detected = self._suspicion_panel:text( { name="suspicion_detected", text=managers.localization:to_upper_text( "hud_detected" ), font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, layer=2, align="center", vertical="center", alpha=0 } )
	...
	local hud_stealth_eye = self._misc_panel:bitmap( { name = "hud_stealth_eye", visible = true, texture = "guis/textures/pd2/hud_stealth_eye", alpha = 0, w = 32, h = 32, valign = "center", blend_mode="add", layer=1 } )
	...
	local hud_stealth_exclam = self._misc_panel:bitmap( { name = "hud_stealth_exclam", visible = true, texture = "guis/textures/pd2/hud_stealth_exclam", alpha = 0, w = 32, h = 32, valign = "center", blend_mode="add", layer=1 } )
...
function HUDSuspicion:animate_eye()
	...
	local animate_func = function( o, self )
		...
		-- local suspicion_bitmap = o:child( "suspicion" )
		-- local suspicion_bg = o:child( "suspicion_bg" )
		...
		while( true ) do
			...
			value = math.lerp( value, wanted_value, 0.9 ) -- dt * 10 )
			c = math.lerp( 1, 0, value ) -- math.lerp( 1, 0, math.clamp( (value - 0.75)*4, 0, 1 ) )
			...
			-- suspicion_bitmap:set_color( Color( 1, c, c ) )  -- Color( 1, c, c ) )
...
--[[
	if type( self._suspicion_ratio ) == "boolean" then
		if self._suspicion_ratio == true then
			SoundDevice:set_rtpc( "suspicion", 0 )
			
			self._suspicion_sound_looping = nil
		elseif( self._suspicion_sound_looping ) then
			SoundDevice:set_rtpc( "suspicion", 0 )
			
			self._suspicion_sound_looping = nil
		end
	elseif type( self._suspicion_ratio ) == "number" then
		if( not self._suspicion_sound_looping ) then
			self._suspicion_sound_looping = true
			self._unit:sound():play( "hud_suspicion_start" )
		end
		
		SoundDevice:set_rtpc( "suspicion", self._suspicion_ratio * 100 )
	end
	]]
...
function HUDSuspicion:feed_value( value )
	...
	--[[
	self._suspicion_panel:child( "suspicion" ):set_alpha( value )
	local c = math.lerp( 1, 0, math.clamp( (value - 0.75)*4, 0, 1 ) )
	self._suspicion_panel:child( "suspicion" ):set_color( Color( 1, c, c ) ) 
	]]
end
