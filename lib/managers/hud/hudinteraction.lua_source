...
function HUDInteraction:init( hud, child_name )
	...
	local interact_text = self._hud_panel:text( { name = self._child_name_text, visible=false, text = "HELLO", valign="center", align="center", layer = 1, color = Color.white, font = tweak_data.hud_present.text_font, font_size = tweak_data.hud_present.text_size, h = 64 } )
	local invalid_text = self._hud_panel:text( { name = self._child_ivalid_name_text, visible=false, text = "HELLO", valign="center", align="center", layer = 3, color = Color( 1, 0.3, 0.3 ), blend_mode = "normal", font = tweak_data.hud_present.text_font, font_size = tweak_data.hud_present.text_size, h = 64 } )
...
function HUDInteraction:show_interaction_bar( current, total )
	...
	-- print( "HUDInteraction:show_interaction_bar", current, total )
	
	self._interact_circle = CircleBitmapGuiObject:new( self._hud_panel, { use_bg = true, radius = self._circle_radius, sides = self._sides, current = self._sides, total = self._sides, color = Color.white:with_alpha( 1 ), blend_mode = "add", layer = 2 } ) 
...
function HUDInteraction:set_interaction_bar_width( current, total )
	...
	-- self._interact_circle:set_current( self._sides * (current/total) )
	self._interact_circle:set_current( (current/total) )
end

function HUDInteraction:hide_interaction_bar( complete )
	-- print( "HUDInteraction:hide_interaction_bar WAS COMPLETE?", complete )
	if complete then
		-- local bitmap = self._hud_panel:bitmap( { texture = "guis/textures/finalframe", blend_mode = "add", align = "center", valign = "center", --[[texture_rect = { 1024 - 128, 1024 - 128, 128, 128 },]] layer = 2 } )
		local bitmap = self._hud_panel:bitmap( { texture = "guis/textures/pd2/hud_progress_active", blend_mode = "add", align = "center", valign = "center", --[[texture_rect = { 1024 - 128, 1024 - 128, 128, 128 },]] layer = 2 } )
		bitmap:set_position( bitmap:parent():w()/2 - bitmap:w()/2, bitmap:parent():h()/2 - bitmap:h()/2 ) -- self._interact_circle:position() )
				
		local radius = 64
		local circle = CircleBitmapGuiObject:new( self._hud_panel, { radius = radius, sides = 64, current = 64, total = 64, color = Color.white:with_alpha( 1 ), blend_mode = "normal", layer = 3 } )
...
function HUDInteraction:set_bar_valid( valid, text_id )
	-- local color = valid and Color.white or Color.red
	-- self._interact_circle:set_color( color )
	...
	-- self._interact_circle:set_alpha( valid and 1 or 1 )
end
...
function HUDInteraction:_animate_interaction_complete( bitmap, circle )
	...
	circle:remove()
	-- print( "HUDInteraction:_animate_interaction_complete done" )
end
