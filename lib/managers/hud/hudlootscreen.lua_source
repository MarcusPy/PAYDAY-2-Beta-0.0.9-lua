...
function HUDLootScreen:create_peer( peers_panel, peer_id )
	...
	local global_value_text = card_info_panel:text( { name="global_value_text", font=small_font, font_size=small_font_size, text=managers.localization:to_upper_text("menu_l_infamous"), color=tweak_data.lootdrop.global_values.infamous.color, blend_mode="add" } )
	...
	local card_nums = { "joker", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ace" }
	local total_cards_w = (panel:w() - peer_info_panel:w() - card_info_panel:w()) - 10
	local card_w = math.round( (total_cards_w-10) / 3 )
	for i=1, 3 do
		local card_panel = panel:panel( { name="card"..i, x=peer_info_panel:right()+(i-1)*card_w+10, y=10, w=card_w-2.5, h=panel:h()-20, halign="scale", valign="scale" } )
		...
		if not is_local_peer then
			downcard:set_size( math.round(128/180*card_panel:h()*0.75), math.round(card_panel:h()*0.75) )
		end 
...
function HUDLootScreen:begin_choose_card( peer_id, card_id )
	...
	local card_nums = { "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ace" }
