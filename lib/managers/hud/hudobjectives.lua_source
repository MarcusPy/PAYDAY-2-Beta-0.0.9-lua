...
function HUDObjectives:init( hud )
	...
	local icon_objectivebox = objectives_panel:bitmap( { halign="left", valign="top", name = "icon_objectivebox", blend_mode = "normal", visible = true, layer = 0, texture = "guis/textures/pd2/hud_icon_objectivebox", x = 0, y = 0, w = 24, h = 24 } )
	
	self._bg_box = HUDBGBox_create( objectives_panel, { w = 200, h = 38, x = 23 + 3, y = 0 } ) -- HUDBGBox:new( objectives_panel, { w = 200, h = 40, y = 20 } )
	
	local objective_text = objectives_panel:text( { name = "objective_text", visible = false, layer = 2, color = Color.white, text="", font_size = tweak_data.hud.active_objective_title_font_size, font = tweak_data.hud.medium_font_noshadow, x = 0, y = 8, align = "left", vertical = "top" } )
	objective_text:set_x( self._bg_box:x() + 8 )
	objective_text:set_y( 6 )
	
	local amount_text = objectives_panel:text( { name = "amount_text", visible = true, layer = 2, color = Color.white, text="1/4", font_size = tweak_data.hud.active_objective_title_font_size, font = tweak_data.hud.medium_font_noshadow, x = 6, y = 0, align = "left", vertical = "top" } )
	amount_text:set_x( objective_text:x() )
	amount_text:set_y( objective_text:y() + objective_text:font_size() - 2 )
end

function HUDObjectives:activate_objective( data )
	-- print( "HUDObjectives:activate_objective", inspect( data ) )
	...
	self._bg_box:stop()
	self._bg_box:animate( callback( nil, _G, "HUDBGBox_animate_open_right" ), 0.66, w + 8*2, callback( self, self, "open_right_done", data.amount and true or false ) )
...
function HUDObjectives:_animate_icon_objectivebox( icon_objectivebox )
	...
	icon_objectivebox:set_y( 0 )
	-- print( " ------- done", icon_objectivebox:y() )
end
