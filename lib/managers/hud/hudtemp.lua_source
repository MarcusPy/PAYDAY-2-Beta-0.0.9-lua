...
function HUDTemp:init( hud )
	...
	self._temp_panel = self._hud_panel:panel( { visible = true, name = "temp_panel", y = 0, valign = "scale", layer = 0 } ) -- valign = {1/3,0}, layer = 3 } )
	
	local throw_instruction = self._temp_panel:text( { name = "throw_instruction", text = "", visible = false, alpha = 0.85, align = "right", vertical = "bottom", halign = "right", valign = "bottom", w = 300, h = 40, layer = 1, x = 8, y = 2, color = Color.white, font = "fonts/font_medium_mf", font_size = 24 } )
	self:set_throw_bag_text()
	-- throw_instruction:set_debug( true )
	local bag_panel = self._temp_panel:panel( { visible = false, name = "bag_panel", halign = "right", valign = "bottom", layer = 10 } )
	-- bag_panel:set_debug( true )
	
	self._bg_box = HUDBGBox_create( bag_panel, { w = 142, h = 56, x = 0, y = 0 } )
	bag_panel:set_size( self._bg_box:size() )
	self._bg_box:text( { name = "bag_text", text = "CARRYING:\nCIRCUIT BOARDS", vertical="left", valign="center", layer = 1, x = 8, y = 2, color = Color.white, font = "fonts/font_medium_mf", font_size = 24 } )
	 
	--[[
	local carry_bag = bag_panel:bitmap( { name = "carry_bag", texture_rect = { 28, 52, 201, 161 }, visible = false, halign = "scale", valign = "scale", texture = "guis/textures/pd2/icon_carrybag", color = Color.white, alpha = 1} )
	carry_bag:set_size( carry_bag:w() * 0.5, carry_bag:h() * 0.5 )
	]]
		
	-- bag_panel:set_size( carry_bag:size() )
	local bag_text = bag_panel:text( { name = "bag_text", visible = false, text = "HEJ", font = "fonts/font_large_mf", align = "center", vertical = "center", font_size = 42, halign = "scale", valign = "scale", color = Color.black, w = 256, h = 128 } )
	...
	self._stamina_panel = self._temp_panel:panel( { visable = true, name = "stamina_panel", layer = 0, w = 16, h = 128, halign = "right", valign = "center", alpha=0 } )
	local stamina_bar_bg = self._stamina_panel:rect( { name = "stamina_bar_bg", color = Color( 0.6, 0.6, 0.6 ), alpha = 0.25 } )
	local low_stamina_bar = self._stamina_panel:rect( { name = "low_stamina_bar", color = Color( 0.6, 0.6, 0.6 ), alpha = 0.0 } )
	local stamina_bar = self._stamina_panel:rect( { name = "stamina_bar", color = Color( 0.6, 0.6, 0.6 ), layer = 1 } )
	local stamina_threshold = self._stamina_panel:rect( { name = "stamina_threshold", color = Color( 1, 1, 1 ), layer = 2, h = 2 } )
	
	self._stamina_panel:rect( { name = "top_border", color = Color(),    layer = 3, h = 2 } ):set_top( 0 )
	self._stamina_panel:rect( { name = "bottom_border", color = Color(), layer = 3, h = 2 } ):set_bottom( 128 )
	self._stamina_panel:rect( { name = "left_border", color = Color(),   layer = 3, w = 2 } ):set_left( 0 )
	self._stamina_panel:rect( { name = "right_border", color = Color(),  layer = 3, w = 2 } ):set_right( 16 )
...
function HUDTemp:_bag_panel_bottom()
	return self._temp_panel:h() - managers.hud:teampanels_height() -- 152 -- Taken from teammate panel height
end
...
function HUDTemp:show_carry_bag( carry_id, value )
	...
	-- bag_text:set_text( utf8.to_upper( "VAS" ) )
	...
	-- self._temp_panel:stop()
end

function HUDTemp:hide_carry_bag()
	local bag_panel = self._temp_panel:child( "bag_panel" ) -- :child( "carry_bag" )
