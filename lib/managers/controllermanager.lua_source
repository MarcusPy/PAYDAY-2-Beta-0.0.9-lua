...
function ControllerManager:set_user_mod( connection_name, params ) -- axis, name, control_id, binding )
...
function ControllerManager:default_controller_connect_change( connected )
	...
	-- if( not connected and not Global.controller_manager.connect_controller_dialog_visible ) then
	if Global.controller_manager.default_wrapper_index then
		...
		if( not connected and not self:_controller_changed_dialog_active() ) then
			--[[if managers.menu:active_menu() then
				managers.menu:active_menu().logic:accept_input( false )
			end]]
...
function ControllerManager:_show_controller_changed_dialog()
	if self:_controller_changed_dialog_active() then -- self._connect_controller_dialog_visible then
		return
	end
...
-- Happens when default wrapper index is set to nil, to prevent dialog get stuck
function ControllerManager:_close_controller_changed_dialog()
	-- if Global.controller_manager.connect_controller_dialog_visible or self._connect_controller_dialog_visible then
