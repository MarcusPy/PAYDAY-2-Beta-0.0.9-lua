...
function SecretAssignmentManager:_setup()
	...
	self._chance = {}
	self._chance.test_interval	= 60 			-- How frequenty to test if new assignment can be given
	self._chance.interval_timer = self._chance.test_interval
	self._chance.chance 		= 5 			-- Means one in five
end
...
function SecretAssignmentManager:update( t, dt )
	if true then return end -- Secret assignments are now disabled
	
	if self._current_assignment then
		if self._current_assignment.timer then -- Timed assignment check
	...
	if Network:is_client() then -- Only server delegates assignments
		return
	end
	...
	if self._has_delegated then -- Only one assignment active
		return
	end
	
	self._chance.interval_timer = self._chance.interval_timer - dt
	if self._chance.interval_timer <= 0 then
		self._chance.interval_timer = self._chance.test_interval
...
-- Can be used from the mission script to enable or disable the chance of an assignment to be delegated
...
function SecretAssignmentManager:unregister_unit( unit, failed )
	...
	for name,data in pairs( tweak_data.secret_assignment_manager ) do
		if name == id then
			...
			elseif data.type == "kill" then
				 if self._assignments[ name ].unit == unit then
				 	if failed and self._assignments[ name ].assigned then -- If assigned, fail the assignment
...
-- Called from server to let know that a target for a kill assignment has been terminated
function SecretAssignmentManager:complete_secret_assignment( name )
	...
	-- Mayby add counters here if several complete needs to happen before completing
	
	self:_complete_assignment( name )
end

-- Called from server to let know that the assignment has failed
function SecretAssignmentManager:failed_secret_assignment( name )
	...
	-- Mayby add counters here if several fails needs to happen before failing
...
-- Called from the delegated peer
...
function SecretAssignmentManager:delegate_assignment()
	...
	if #can_be_activated == 0 then -- No assignments to delegate 
		-- print( "no assignments to delegate" )
		return
	end
	...
	if not peer then -- No good peers found
		return
	end
	...
	if tweak_data.secret_assignment_manager[ name ].type == "kill" then
		-- Since only server really knows when a character is killed, he needs to register the callback and then call the correct peer
		self._assignments[ name ].unit:unit_data().mission_element:add_event_callback( "death", callback( self, self, "target_killed" ) )
	end
	peer:send_queued_sync( "assign_secret_assignment", name ) --, self._assignments[ name ].unit or self._assignments[ name ].available_units[1] )
...
-- Return value true means the assignment is ok by the level
...
function SecretAssignmentManager:_get_peer()
	...
	for id,member in pairs( managers.network:game():all_members() ) do
		-- Discard members who has no unit or is in mask_off state
...
function SecretAssignmentManager:assign( name )
	...
	elseif data.type == "kill" then
		-- Don't really do anything here since the server takes care of the death callbacks from characters
	end 
end

-- Randomly selectes amount of units to activate interact on
...
--[[
function SecretAssignmentManager:test_give_assignment()
	local can_be_activated = {}
	for name,data in pairs( self._assignments ) do
		if data.can_be_activated and not self._assignments[ name ].assigned and not self._assignments[ name ].completed then
			table.insert( can_be_activated, name )
		end
	end
	...
	if tweak_data.secret_assignment_manager[ name ].type == "interact" then
		local unit = self._assignments[ name ].available_units[ math.random( #self._assignments[ name ].available_units ) ]
		unit:interaction():set_assignment( name )
		unit:interaction():set_active( true )
	elseif tweak_data.secret_assignment_manager[ name ].type == "kill" then
	end 
end]]

-- Return a sorted name table of all assignments
