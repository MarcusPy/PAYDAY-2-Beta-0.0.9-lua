...
function GroupAIStateStreet:_init_misc_data()
	...
	if managers.navigation:is_data_ready() then
		...
		--self._criminal_center = nil -- mid position between criminals
		...
		for u_key, u_data in pairs( self._police ) do
			if not u_data.assigned_area then
				...
				self:set_enemy_assigned( all_areas[ nav_seg ], u_key )	-- assign
			end
		end
...
function GroupAIStateStreet:_begin_new_tasks( t )
	...
	if self._wave_mode == "blockade" then
		...
		--print( "blockade_data.next_dispatch_t", blockade_data.next_dispatch_t and blockade_data.next_dispatch_t - t )
...
function GroupAIStateStreet:_begin_blockade_task( t )
	...
	if not event then
		--print( "[GroupAIStateStreet:_begin_blockade_task] failed to find suitable blockade event" )
		return
	end
	--Application:set_pause( true )
	--Application:draw_sphere( best_blockade_pos, 230, 0.6,0.6,0 )
	...
	task_data.dispatch = function ()
		--print( "blockade -----------ANTICIPATION-------------" )
...
function GroupAIStateStreet:_upd_blockade_task( t )
	...
	if not ( phase or task_data.passive ) and task_data.next_dispatch_t and t > task_data.next_dispatch_t then
		task_data.dispatch()
		phase = task_data.phase
	end
	
	--Application:draw_cylinder( target_pos, target_pos + math.UP * 1000, 230, 0.6,0.6,0 )
	--Application:set_pause( true )
	
	-- find out which players are in front of the blockade area
	...
	if has_irrelevant_players then
		...
		if event then
			...
			--print( "[GroupAIStateStreet:_upd_blockade_task] has_irrelevant_players. Found new event" )
		else
			--print( "[GroupAIStateStreet:_upd_blockade_task] has_irrelevant_players. Failed to find new event" )
	...
	for _, blockade_point in ipairs( event.blockade_points ) do
		...
		for c_key, c_record in pairs( relevant_players ) do
			...
			if not failed_to_change_event and pl_dis < compromise_dis then
				local event = self:_find_blockade_event( t )
				if event then
					task_data.event = event
					task_data.target_area = event.nav_seg
					task_data.mission_fwd = event.rot:y()
					self:_use_spawn_event( event )
					--print( "[GroupAIStateStreet:_upd_blockade_task] Blockade compromised. Found new event", event, inspect( event ) )
					return
				else
					--print( "[GroupAIStateStreet:_upd_blockade_task] Blockade compromised. Failed to find new event" )
				end
	...
	elseif phase == "sustain" then
		if t > task_data.phase_end_t then
			if not self._hunt_mode then	-- We do not expire during hunt mode
				...
				phase = task_data.phase
				task_data.phase_end_t = nil
				task_data.phase_end_t = t + 10 -- allow 5 seconds before ending assault
			end
		end
...
--				print( "assault task clear" )
	...
	-- count frontal and rear enemies
	...
	for u_key, u_data in pairs( self._police ) do	-- count how many enemies we have at the front and at the back
		if u_data.assigned_area then
			if cops_in_area[ u_key ] and u_data.unit:brain():objective() and u_data.unit:brain():objective().type == "defend_area" then -- defender
				table.insert( blocker_pigs, u_key )
				--Application:draw_sphere( u_data.m_pos, 60, 0, 0.6, 0 )
			else
				front_pigs = front_pigs + 1
			end
		--else
			--Application:draw_sphere( u_data.m_pos, 60, 0, 0, 0 )
		end
	end
	...
	--print( "undershot", undershot, "spawn_threshold", spawn_threshold )
	if undershot > 0 then	-- we have too few defenders
		if spawn_threshold > 0 then
			if not self:_try_use_task_spawn_event( t, task_data.target_area, "assault" ) then
				...
				--print( "nr_wanted", nr_wanted, "#spawn_points", spawn_points and #spawn_points )
				if spawn_points then
					...
					for _, sp_data in ipairs( spawn_points ) do -- assign a random defend position for each spawned enemy
	...
	elseif undershot < 0 then	-- we have too many defenders ( overshot )
		for i = 1, -undershot, 1 do
			local u_key = blocker_pigs[ #blocker_pigs ]
			table.remove( blocker_pigs )
			local u_data = self._police[ u_key ]
			u_data.unit:brain():set_objective( nil ) -- nil for now. he will get an objective later on in this function
		end
	end
		
	if not self._task_data.regroup.active then -- do not spawn under regroup
		if spawn_threshold > 0 then	-- spawn rear assault
			local flank_assault = task_data.flank_assault
			if flank_assault.sneak_unit_key then
				if not self._police[ flank_assault.sneak_unit_key ] then
					flank_assault.sneak_unit_key = nil
					flank_assault.next_dispatch_t = t + math.random( 45, 300 )
				end
			end
			if not flank_assault.sneak_unit_key and flank_assault.next_dispatch_t < t then
				...
				if spawn_points then
					...
					for c_key, c_data in pairs( relevant_players ) do	-- attack the player furthest back
						if not c_data.status then
					...
					--[[Application:draw_cylinder( target_pos, target_pos + task_data.mission_fwd * 1000, 30, 0, 1, 0 )
					Application:draw_sphere( target_pos, 100, 0, 1, 0 )
					Application:draw_cylinder( sp_data.pos, sp_data.pos + math.UP * 5000, 40, 1, 0, 0 )
					Application:set_pause( true )]]
					local spawned_units = {}
		...
		if spawn_threshold > 0 and ( phase == "build" or phase == "sustain" ) then	-- spawn frontal assault
			--print( "front_pigs", front_pigs, "blocker_pigs", #blocker_pigs )
			local wanted_fwd = math.min( task_data.force_required.frontal - front_pigs, spawn_threshold )
			if wanted_fwd > 0 then
				if not self:_try_use_task_spawn_event( t, task_data.target_area, "assault" ) then
					...
					--print( "wanted_fwd", wanted_fwd, "spawn_points", spawn_points and inspect( spawn_points ) )
					if spawn_points then
						local objectives = {}
						local objective = { 
							type = "investigate_area"
	...
	-- reassign lost cops
	for u_key, u_data in pairs( self._police ) do
		if u_data.assigned_area and u_data.unit:brain():is_available_for_assignment( { type = "investigate_area", interrupt_dis = 700, interrupt_health = 0.75 } ) then
		...
		if event_data.task_type == "blockade" and not event_data.broken then
			...
			if not invalid then
				for c_key, c_record in pairs( self._player_criminals ) do
					if not event_data.relevant_areas[ c_record.tracker:nav_segment() ] then -- a player is behind the blockade. not good
						invalid = true
						break
					end
				end
				if not invalid then
					for _, blockade_point in ipairs( event_data.blockade_points ) do
						...
						local invalid
						for c_key, c_record in pairs( self._player_criminals ) do
							if mvec3_dis( c_record.m_pos, blockade_pos ) < min_dis then	-- a player is too near the blockade. not good
								invalid = true
								break
							end
						end
...
--		print( "-----------REENFORCE-------------" )
		...
		if undershot < 0 then -- we have more than needed. free some
			for i = 0, undershot, 1 do
				local u_key, u_data = next( target_area_data.police.units )
				local unit = u_data.unit
				local objective = u_data.unit:brain():objective()
				local seg = unit:movement():nav_tracker():nav_segment()
				local new_objective = { type = "free", nav_seg = seg, attitude = objective.attitude, stance = objective.stance, scan = true }
				self:set_enemy_assigned( self._area_data[ seg ], u_key )
				unit:brain():set_objective( new_objective )
			end
		elseif undershot > 0 then -- we need reenforcement
			...
			if undershot > 0 then -- we will spawn some units because we could not fill from existing ones
				local spawn_points = GroupAIStateBesiege._find_spawn_points_near_area( self, target_area, undershot, nil, 4000 )
				--print( "#spawn_points", spawn_points and #spawn_points )
				if spawn_points then
					local objective = { 
						type = "defend_area"
						,nav_seg = target_area
...
-----------------------------------------------------------------------------------
-- choose a direction and attack from there
...
function GroupAIStateStreet:_upd_assault_task( t )
	...
	elseif task_phase == "sustain" then
		if t > task_data.phase_end_t then
			if not self._hunt_mode then	-- We do not expire during hunt mode
				...
				task_data.use_smoke_timer = t + 20 -- Just set this somewhere past assault mode to make sure
				task_data.phase_end_t = nil
				task_data.phase_end_t = t + 10 -- allow 10 seconds before ending assault
			end
		end
	else -- task_phase == "fade"
		if t > task_data.phase_end_t - 8 and not task_data.said_retreat then
			if self._drama_data.zone ~= "high" then
				task_data.said_retreat = true
				self:_police_announce_retreat()
	...
	-- count defensive, aggressive, idle and unavailable cops
	...
	for u_key, u_data in pairs( self._police ) do
		if u_data.assigned_area then
			...
			elseif objective.type == "defend_area" then
				if not ( u_data.assigned_area.factors and u_data.assigned_area.factors.force ) then
					defensive_cops[ u_key ] = u_data
					nr_defensive_cops = nr_defensive_cops + 1
				end
	...
	--print( "nr_agressive_cops", nr_agressive_cops, "nr_defensive_cops", nr_defensive_cops )
	-- Check if our target area is still valid
	local target_area = task_data.target_area
	...
	if task_phase == "anticipation" then
		local spawn_threshold = math.max( 0, self._police_force_max - self._police_force - 5 ) -- leave a few places for when build begins
		if spawn_threshold > 0 then -- If this is positive then we have too few cops in the level. spawn instead of re-assigning
		...
		-- TODO: Is this really appropriate? Just reassigning defenders to avoid doesn't really feel like anticipation
		if task_phase == "anticipation" then
			objective_attitude = "avoid"
			spawn_threshold = math.max(0, spawn_threshold - 5) -- leave a few places for when build begins
		end
		
		
		-- fill aggressive cops by reassigning and spawning
		-- if it's anticipation the cops should not yet be agressive
		...
		if undershot_aggressive > 0 then	-- we need reenforcements
			local u_key, u_data
			while undershot_aggressive > 0 and nr_defensive_cops > 0 do	-- first try reassigning defenders to attack
				u_key, u_data = next( defensive_cops, u_key )
				if not u_key then
					break
				end
				local unit = u_data.unit
				if not u_data.follower and u_data.unit:brain():is_available_for_assignment( { type = "investigate_area", interrupt_dis = objective_interrupt_dis, interrupt_health = objective_interrupt_health } ) then
			...
			if undershot_aggressive > 0 and spawn_threshold > 0 then	-- If the amount reassigned was not enough, spawn some cops
		...
		elseif undershot_aggressive < 0 and task_phase ~= "fade" then	-- we need to releave some aggresive attackers
			local u_key, u_data
			while undershot_aggressive < 0 do
				...
				if not ( u_data.follower or u_data.unit:brain()._important ) then -- do not releave important units
					local unit = u_data.unit
					...
					if old_objective and u_data.unit:brain():is_available_for_assignment() then
						...
						defensive_cops[ u_key ] = u_data
						nr_defensive_cops = nr_defensive_cops + 1
						--print( "reassigned to avoid" )
					end
				end
			end
		end
		
		-- fill defensive cops by reassigning and spawning
		...
		if undershot_defensive > 0 and spawn_threshold > 0 then	-- If the amount of backup_cops is not enough, spawn some more
		...
		elseif task_phase == "fade" then
			for u_key, u_data in pairs( defensive_cops ) do
				local unit = u_data.unit
				if not u_data.follower and u_data.unit:brain():is_available_for_assignment( { type = "investigate_area", interrupt_dis = -1, interrupt_health = 1 } ) then
					local closest_dis, closest_criminal_data
					for c_key, c_data in pairs( healthy_criminals ) do
						local my_dis = mvec3_dis( c_data.m_pos, u_data.m_pos )
						if not closest_dis or closest_dis > my_dis then
		...
		if task_data.use_smoke and not self:is_smoke_grenade_active() then
			...
			if shoot_smoke then
				...
				if Network:is_server() then -- Is this even needed here?
					local ignore_ctrl
...
function GroupAIStateStreet:_find_spawn_points_behind_pos( start_nav_seg, target_pos, nr_wanted, fwd, nav_segs, max_dis )
	...
	repeat
			...
			for _, sp_data in ipairs( spawn_points ) do
				if sp_data.delay_t <= t then
					...
					if not max_dis or my_dis < max_dis then
						...
						--[[else
							Application:draw_sphere( sp_data.pos, 200, 0,0,0.5)
							Application:draw_cylinder( sp_data.pos, target_pos, 10, 0,0,0.5)
							Application:draw_cylinder( sp_data.pos, sp_data.pos + fwd * 500, 5, 0,1,1)]]
						end
					--[[else -- too far
						Application:draw_sphere( sp_data.pos, 200, 0.6,0.6,0.6 )
						Application:draw_cylinder( sp_data.pos, managers.navigation._nav_segments[ start_nav_seg ].pos, 10, 0,0,1)]]
					end
				--[[else	-- delayed
					Application:draw_sphere( sp_data.pos, 200, 0,0,0 )
					Application:draw_cylinder( sp_data.pos, managers.navigation._nav_segments[ start_nav_seg ].pos, 10, 0,0,1)]]
				end
			end
		end
		...
		for _seg_id, nav_seg_data in pairs( nav_segs or all_nav_segs ) do
			...
			if nav_seg_data.neighbours and not ( found_segs[ _seg_id ] or nav_seg_data.disabled ) and nav_seg_data.neighbours[ search_seg ] then
...
function GroupAIStateStreet:_spawn_cops_with_objective( area, spawn_points, objective, unit_weights )
	...
	for i_sp = 1, #spawn_points do
		...
		if unit_name then
			...
			--Application:draw_sphere( spawned_enemy:position(), 50, 1,0,1 )
...
function GroupAIStateStreet:_end_regroup_task( regroup_task )
	...
	--print( "regroup task clear self._wave_mode", self._wave_mode )
...
-----------------------------------------------------------------------------------
-- Inform neighbouring areas that they have a new neighbour or that an neighbour is no longer available
...
function GroupAIStateStreet:set_area_min_police_force( id, force, pos )
	...
	if force and force > 0 then
		...
		-- update the already existing task on the area
		for i_task, task_data in ipairs( self._task_data.reenforce.tasks ) do
...
function GroupAIStateStreet:set_wave_mode( flag )
	...
	if flag == "blockade" then
		...
		if old_wave_mode ~= flag then
			...
			if next_dispatch_t then -- pass planned assault time to the new event
				task_data.assault.next_dispatch_t = nil
				task_data.blockade.next_dispatch_t = next_dispatch_t
			elseif not task_data.regroup.active then	-- no assault scheduled. schedule one for now
	...
	elseif flag == "assault" then
		if old_wave_mode ~= flag then
			...
			self:_begin_assault_task( self._t ) -- setting assault on twice forces an assault
		end
...
function GroupAIStateStreet:add_spawn_event( id, event_data )
	...
	if event_data.task_type == "blockade" then -- find the closest ambush_point for this event
		...
		event_data.relevant_areas = relevant_nav_segments -- table: area_id, true
	end
end
...
function GroupAIStateStreet:is_detection_persistent()
	return self._task_data.assault.active or self._task_data.regroup.active and self._task_data.regroup.phase or self._task_data.blockade.active and self._task_data.blockade.phase
end
