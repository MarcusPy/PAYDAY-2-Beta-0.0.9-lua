	...
	if ai_group == "enemies" then
		for e_key, enemy_unit_data in pairs( self._police ) do
			if enemy_unit_data.assigned_area and not ( so_administered and so_administered[ e_key ] ) then
				if enemy_unit_data.unit:brain():is_available_for_assignment( so_objective ) 
	...
	elseif ai_group == "friendlies" then
		for u_key, u_unit_data in pairs( self._ai_criminals ) do
			if not ( so_administered and so_administered[ u_key ] ) then
				if u_unit_data.unit:brain():is_available_for_assignment( so_objective ) and (not so_data.verification_clbk or so_data.verification_clbk( u_unit_data.unit ))
