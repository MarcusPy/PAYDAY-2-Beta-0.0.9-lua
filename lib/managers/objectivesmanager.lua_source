...
function ObjectivesManager:update( t, dt )	
	for id,data in pairs( self._remind_objectives ) do
		if data.next_t < t then
			-- data.next_t = t + self.REMINDER_INTERVAL
...
-- Basicly same as activate objective but with another presentation text
...
-- Removes an objective without completing it or giving any messages
function ObjectivesManager:remove_objective( id, load_data )
	...
	local objective = self._objectives[ id ]
	managers.hud:complete_objective( { id = id, text = objective.text, remove = true } )
	...
	if not load_data then
		...
		if not self._active_objectives[ id ] then
			if not self._completed_objectives[ id ] then
				self._completed_objectives[ id ] = self._objectives[ id ] -- complete the objective to prevent bugs
		...
		-- Should also check if there are subobjectives?
	end
		
	local objective = self._objectives[ id ]
	if objective.amount then -- If the objective has an amount counter, check if the objective is completed and change the hud info
		objective.current_amount = objective.current_amount + 1
		managers.hud:update_amount_objective( { id = id, text = objective.text, amount_text = objective.amount_text, amount = objective.amount, current_amount = objective.current_amount } )
		if objective.current_amount < objective.amount then
			self:_remind_objetive( id, "mission_objective_updated" )
			return
		end
		objective.current_amount = 0 -- Reset for testing
	end
	
	-- Get XP for the objective
	-- local xp = self:_get_xp( Global.level_data and Global.level_data.level_id, id )
	
	managers.hud:complete_objective( { id = id, text = objective.text } )
	-- local text = managers.localization:text( "mission_objective_completed" )..objective.text
	-- managers.hud:present_mid_text( { text = text, time = 4, icon = nil, event = "stinger_levelup" } )
	managers.statistics:objective_completed()
	-- managers.experience:perform_action( "objective_completed" )
...
function ObjectivesManager:complete_sub_objective( id, sub_id, load_data )
	...
	managers.hud:complete_sub_objective( { text = objective.text, sub_id = sub_id } )
...
-- Returns the total amount of objectives for a level based on the level id
	...
	if( state ) then
		...
		for name,save_data in pairs( state.objective_map ) do
			...
			if( save_data.active ) then
				self:activate_objective( name, { current_amount = save_data.current_amount, amount = save_data.amount } )
				for sub_id,completed in pairs( save_data.sub_objective ) do
					if completed then
						self:complete_sub_objective( name, sub_id, {} )
					end
				end
				-- self._active_objectives[ name ] = objective_data
			end
