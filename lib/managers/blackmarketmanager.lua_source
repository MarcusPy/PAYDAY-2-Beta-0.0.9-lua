...
function BlackMarketManager:_setup_weapons()
	...
	for weapon,data in pairs( tweak_data.weapon ) do
		if data.autohit then
			...
			weapons[ weapon ] = { unlocked = false, factory_id = factory_id, selection_index = selection_index, condition = condition }
		end
	end
end
...
function BlackMarketManager:on_sell_weapon_part( category, slot, global_value, part_id )
	if self:remove_weapon_part( category, slot, global_value, part_id ) then
		-- managers.money:on_sell_weapon_part( part_id ) -- needs global value, and also on mod part ( remove to default )
	end
end
...
function BlackMarketManager:load( data )
	...
	if data.blackmarket then
		...
		-- Verify loaded data, this will be changed alot
		-- Mask
		for mask,_ in pairs( tweak_data.blackmarket.masks ) do
			if not self._global.masks[ mask ] then
				self._global.masks[ mask ] = { unlocked = true, owned = true, equipped = false }
			end
		end
		...
		for armor,_ in pairs( tweak_data.blackmarket.armors ) do
			if not self._global.armors[ armor ] then
				self._global.armors[ armor ] = { unlocked = true, owned = true, equipped = false }
			end
		end
		...
		for deployable,_ in pairs( tweak_data.blackmarket.deployables ) do
			if not self._global.deployables[ deployable ] then
				self._global.deployables[ deployable ] = { unlocked = true, owned = true, equipped = false }
			end
		end
		for deployable,_ in pairs( clone( self._global.deployables ) ) do
			if not tweak_data.blackmarket.deployables[ deployable ] then
				self._global.deployables[ deployable ] = nil
			end
		end]]
		...
		for weapon,data in pairs( tweak_data.weapon ) do
			if data.autohit then -- Is player weapon
				if not self._global.weapons[ weapon ] then
					...
					local selection_index = data.use_data.selection_index
					local factory_id = managers.weapon_factory:get_factory_id_by_weapon_id( weapon )
					self._global.weapons[ weapon ] = { 	unlocked = false,
		...
		-- managers.savefile:add_load_sequence_done_callback_handler( callback( self, self, "_verify_dlc_items" ) )
