...
function FadeoutGuiObject:init( params )
	...
	-- self._panel:rect( { color = fade_color, alpha = 1 } )
	
	if show_loding_icon then
		local loading_icon = self._panel:bitmap( { name="loading_icon", texture=loading_texture } )
	...
	local fade_out_animation = function( panel )
