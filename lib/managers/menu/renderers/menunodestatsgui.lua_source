...
function MenuNodeStatsGui:_setup_stats( node )
	...
	self:_add_stats( { topic = managers.localization:text( "menu_stats_favourite_campaign" ), data = string.upper( managers.statistics:favourite_level() ), type = "text" })
	self:_add_stats( { topic = managers.localization:text( "menu_stats_total_completed_campaigns" ), data = ""..managers.statistics:total_completed_campaigns(), type = "text" })
	self:_add_stats( { topic = managers.localization:text( "menu_stats_total_completed_objectives" ), data = ""..managers.statistics:total_completed_objectives(), type = "text" })
	...
	self:_add_stats( { topic = managers.localization:text( "menu_stats_favourite_weapon" ), data = ""..string.upper( managers.statistics:favourite_weapon() ), type = "text" })
	self:_add_stats( { topic = managers.localization:text( "menu_stats_hit_accuracy" ), data = ""..managers.statistics:hit_accuracy().."%", type = "text" })
	self:_add_stats( { topic = managers.localization:text( "menu_stats_total_kills" ), data = ""..managers.statistics:total_kills(), type = "text" })
	self:_add_stats( { topic = managers.localization:text( "menu_stats_total_head_shots" ), data = ""..managers.statistics:total_head_shots(), type = "text" })
	--[[self:_add_stats( { topic = "SOMETHING:", data = 16/100, type = "progress" })
	self:_add_stats( { topic = "ZERO:", data = 0/100, type = "progress" })
	self:_add_stats( { topic = "ALL YEAH:", data = 1002/1002, type = "progress" })]]
	...
	if SystemInfo:platform() == Idstring( "WIN32" ) then
		...
		local text = panel:text( {
									...
									x = safe_rect.x, y = 0, w = safe_rect.width,
...
function MenuNodeStatsGui:_add_stats( params )
	...
	local topic = panel:text( {
								font_size = tweak_data.menu.stats_font_size,
								x = 0, y = 0, w = self:_left_align(),
								align="right", halign="right", vertical="center",
								font = self.font,
								color = self.row_item_color,
								layer = self.layers.items,
								text = params.topic,
	...
	if params.type == "progress" then
		...
		local text = panel:text( {
									font_size = tweak_data.menu.stats_font_size,
									x = self:_right_align(), y = 0, h = h, w = bg:w(),
