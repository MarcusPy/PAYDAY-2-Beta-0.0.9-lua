...
function StatsTabItem:init( panel, text, i )
	...
	self._tab_text = self._main_panel:text( { name="tab_text_"..tostring(self._index), text=text, h=32, x=offset+5, align="center", vertical="center", font_size=tweak_data.menu.pd2_medium_font_size, font=tweak_data.menu.pd2_medium_font, color=tweak_data.screen_colors.button_stage_3, layer=1, blend_mode="add" } )
	
	local x, y, w, h = self._tab_text:text_rect()
	self._tab_text:set_size( w + 15, h + 10 )
	
	self._select_rect = self._main_panel:bitmap( { name="tab_select_rect_"..tostring(self._index), texture="guis/textures/pd2/shared_tab_box", layer=0, color=tweak_data.screen_colors.text, visible=false } )
...
function StatsTabItem:set_stats( stats_data )
	...
	for i, stat in ipairs( self._stats ) do
		local new_stat_panel = self._panel:panel( { name=tostring(stat), h=tweak_data.menu.pd2_medium_font_size, y=10 } )
		local desc_text = new_stat_panel:text( { name="desc", text=managers.localization:to_upper_text( "victory_" .. stat ), font_size=tweak_data.menu.pd2_small_font_size, font=tweak_data.menu.pd2_small_font, color=tweak_data.screen_colors.text, align="right", vertical="top", w=math.round(self._panel:w()/2)-5-10, x=10, wrap=true, word_wrap=true } )
		local stat_text = new_stat_panel:text( { name="stat", text="", font_size=tweak_data.menu.pd2_small_font_size, font=tweak_data.menu.pd2_small_font, color=tweak_data.screen_colors.text, align="left", vertical="top", w=math.round(self._panel:w()/2)-10, x=math.round(self._panel:w()/2)+5, wrap=true, word_wrap=true } )
		...
		-- widest_text = math.max( widest_text, w )
	...
	local desc = nil
	local stat = nil
	for index, stat_panel in ipairs( self._panel:children() ) do
		desc = stat_panel:child("desc")
		desc:set_w( widest_text )
		desc:set_align( "right" )
				
		stat = stat_panel:child("stat")
		stat:set_left( desc:right() + 5 )
	end
	]]
	-- self._panel:set_left( self._main_panel:w() / 2 - widest_text ) 
end
...
function StatsTabItem:feed_statistics( stats_data )
	...
	for i, stat in ipairs( self._stats ) do
		if stats_data[stat] then
			...
			--[[
			text_dissected = utf8.characters( text )
			start_ci = {}
			end_ci = {}
			first_ci = true
			...
			stat_text:clear_range_color( 1, utf8.len(text) )
				
			if #start_ci ~= # end_ci then
				Application:error( "StatsTabItem: Not even amount of ##'s in :set_info_text() string!", #start_ci, #end_ci )
			else
				for i=1, #start_ci do
					stat_text:set_range_color( start_ci[i], end_ci[i], tweak_data.screen_colors.resource )
				end
			end
			]]
			stat_text:set_text( text )
...
function StatsTabItem.animate_select(o)
	...
	over( math.abs(100-size)/100, function(p) local s = math.lerp( size, 100, p ) o:set_size( s, s ) o:set_center( center_x, center_y ) end )
end
...
function StatsTabItem.animate_deselect(o)
	...
	over( math.abs(65-size)/100, function(p) local s = math.lerp( size, 65, p ) o:set_size( s, s ) o:set_center( center_x, center_y ) end )
end
...
function StageEndScreenGui:init( saferect_ws, fullrect_ws, statistics_data )
	...
	self._panel = self._safe_workspace:panel():panel( { w=self._safe_workspace:panel():w()/2-10, h=self._safe_workspace:panel():h()*0.5-10, layer=6 } )
	self._panel:set_right( self._safe_workspace:panel():w() )
	self._panel:set_bottom( self._safe_workspace:panel():h() )
	
	self._continue_button = self._panel:text( { name="ready_button", text=utf8.to_upper( managers.localization:text( "menu_es_calculating_experience", { CONTINUE = managers.localization:btn_macro( "continue" ) } ) ), h=32, align="right", vertical="center", font_size=tweak_data.menu.pd2_large_font_size, font=tweak_data.menu.pd2_large_font, color=tweak_data.screen_colors.button_stage_3, layer=1 } )
	...
	local big_text = self._fullscreen_panel:text( { name="continue_big_text", text=utf8.to_upper( managers.localization:text( "menu_es_calculating_experience", { CONTINUE = managers.localization:btn_macro( "continue" ) } ) ), h=90, align="right", vertical="bottom", font_size=tweak_data.menu.pd2_massive_font_size, font=tweak_data.menu.pd2_massive_font, color=tweak_data.screen_colors.button_stage_3, alpha=0.4 } )
...
--[[
	self._boost_item	= StatsTabItem:new( self._panel, utf8.to_upper( managers.localization:text( "menu_es_boost" ) ), 1 )
	self._boost_item:set_stats( {
		"most_downs",
		-- "most_downs_score",
	...
	table.insert( self._items, self._boost_item )
	
	self._crew_item	= StatsTabItem:new( self._panel, utf8.to_upper( managers.localization:text( "menu_es_crew" ) ), 2 )
	self._crew_item:set_stats( { 
		"group_total_downed",
		"group_hit_accuracy",
		"criminals_finished",
	} )
	table.insert( self._items, self._crew_item )
		
		
	self._personal_item	= StatsTabItem:new( self._panel, utf8.to_upper( managers.localization:text( "menu_es_personal" ) ), 3 )
	self._personal_item:set_stats( {
		"total_head_shots",
		"total_kills",
		"total_specials_kills",
	...
	table.insert( self._items, self._personal_item )
	
	
	self._the_bad_item	= StatsTabItem:new( self._panel, utf8.to_upper( managers.localization:text( "menu_es_bad" ) ), 4 )
	self._the_bad_item:set_stats( {
		"civilians_killed_penalty",
		"total_downed",
		
	} )
	table.insert( self._items, self._the_bad_item )
	
	
	self._the_other_item	= StatsTabItem:new( self._panel, utf8.to_upper( managers.localization:text( "menu_es_other" ) ), 5 )
	self._the_other_item:set_stats( {
		"time_played",
	...
	table.insert( self._items, self._the_other_item )
	]]
...
function StageEndScreenGui:play_bain_debrief()
	...
	if outro_event then
		managers.briefing:post_event( outro_event, { show_subtitle=false, listener={ end_of_event=true, clbk=callback( self, self, "bain_debrief_end_callback" ) } } )
		
		if managers.menu:is_console() then
			managers.briefing:add_listener( { marker=true, clbk=callback( self, self, "console_subtitle_callback" ) } )
		end
	else
...
function StageEndScreenGui:console_subtitle_callback( event, string_id, duration, cookie )
	if not self._console_subtitle_panel then
		...
		self._console_subtitle_panel:text( { name="subtitle_text", font=tweak_data.menu.pd2_medium_font, font_size=tweak_data.menu.pd2_medium_font_size, text="", align="center", vertical="bottom", wrap=true, word_wrap=true } )
	end
...
--[[
	if managers.job:on_last_stage() then
		...
		if job_data and job_data.debrief_event then
			local variant = managers.groupai:state():endscreen_variant() or 0
			local level_data = Global.level_data.level_id and tweak_data.levels[ Global.level_data.level_id ]
			local outro_event = level_data and ( variant == 0 and level_data.intro_event or level_data.outro_event[ variant ] )
			
			
			managers.briefing:post_event( outro_event ) -- )job_data.debrief_event )
			-- self._post_event = managers.menu:post_event( job_data.debrief_event )
		end
	end
]]

function StageEndScreenGui:update( t, dt )
	if self._bain_debrief_t then
		if self._bain_debrief_t < t then
			self._bain_debrief_t = nil
			self:play_bain_debrief()
		end
	end
	if self._contact_debrief_t then
		if self._contact_debrief_t < t then
...
function StageEndScreenGui:feed_statistics( data )
	...
	data.completed_ratio = (data.success and managers.statistics:started_session_from_beginning() and 100) 
								or (data.total_objectives ~= 0 and (math.round( ( managers.statistics:completed_objectives()/data.total_objectives ) * 100 )) or 0 )
	data.completed_objectives = managers.localization:text( "menu_completed_objectives_of", { COMPLETED=managers.statistics:completed_objectives(), TOTAL=data.total_objectives, PERCENT=data.completed_ratio } )
	...
	-- data.objectives_bonus_xp = math.round( data.total_objectives ~= 0 and ((data.completed_objectives/data.total_objectives) * tweak_data.experience_manager.total_objectives_finished) or 0 )
	-- data.criminals_bonus_xp = data.criminals_finished * tweak_data.experience_manager.total_criminals_finished
	-- data.civilians_penalty_xp = data.total_civilian_kills * tweak_data.experience_manager.civilians_killed
...
function StageEndScreenGui:next_tab( no_sound )
	...
	-- if( self._selected_item > #self._items ) then
...
function StageEndScreenGui:prev_tab( no_sound )
	...
	-- 	self._selected_item = #self._items
	-- end
	self._items[self._selected_item]:select()
...
function StageEndScreenGui:mouse_pressed( button, x, y )
	...
	if( not self._button_not_clickable and self._continue_button:inside( x, y ) ) then
