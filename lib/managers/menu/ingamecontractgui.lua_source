...
function IngameContractGui:init( ws )
	...
	local contract_text = self._panel:text( { layer = 1, rotation = 360, font=tweak_data.menu.pd2_large_font, font_size=tweak_data.menu.pd2_large_font_size, text="", color=tweak_data.screen_colors.text } )
	...
	if job_data then
		...
		for i = 1, 10 do -- job_and_difficulty_stars do
			...
			local star = text_panel:bitmap( { texture="guis/textures/pd2/mission_briefing/difficulty_icons", texture_rect=filled_star_rect, x=x, y=0, w=20, h=20, alpha=alpha, color = color } )
		...
		local stage_cash = text_panel:text( { font=tweak_data.menu.pd2_small_font, font_size=font_size, text=tostring(num_days) .. " x " .. managers.experience:cash_string(money_stage_stars), color=tweak_data.screen_colors.text } )
