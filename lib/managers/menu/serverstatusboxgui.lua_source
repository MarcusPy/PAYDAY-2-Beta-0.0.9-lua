...
function ServerStatusBoxGui:init( ws, title, text, content_data, config )
	...
	config.y = config.y or 1 -- (y - config.h - CoreMenuRenderer.Renderer.border_height)
...
function ServerStatusBoxGui:_create_text_box( ws, title, text, content_data, config )
	...
	local server_panel = self._scroll_panel:panel( { name = "server_panel", x = 0, h = 60, w = self._scroll_panel:w(), layer = 1 } )
	
	local font_size = tweak_data.menu.lobby_info_font_size
	local server_title = server_panel:text( { visible = nil, name = "server_title", text = string.upper( managers.localization:text( "menu_lobby_server_title" ) ), font = tweak_data.menu.default_font, font_size = font_size, align = "left", vertical = "center", w = 256, h = font_size, layer = 1} )
	local server_text = server_panel:text( { visible = nil, name = "server_text", text = string.upper(""..server_name), font = tweak_data.menu.default_font, color = tweak_data.hud.prime_color, font_size = font_size, align = "left", vertical = "center", w = 256, h = font_size, layer = 1} )
	
	local server_state_title = server_panel:text( { visible = nil, name = "server_state_title", text = string.upper( managers.localization:text( "menu_lobby_server_state_title" ) ), font = tweak_data.menu.default_font, font_size = font_size, align = "left", vertical = "center", w = 256, h = font_size, layer = 1} )
	local server_state_text = server_panel:text( { visible = nil, name = "server_state_text", text = string.upper( managers.localization:text( self._server_state_string_id or "menu_lobby_server_state_in_lobby" ) ), font = tweak_data.menu.default_font, color = tweak_data.hud.prime_color, font_size = font_size, align = "left", vertical = "center", w = 256, h = font_size, layer = 1} )
