...
function CrimeNetContractGui:init( ws, fullscreen_ws, node )
	...
	self._contact_text_header = self._panel:text( { text=managers.localization:to_upper_text( "menu_cn_contract_title", { job=managers.localization:text(narrative.name_id) } ), align="left", vertical="top", font_size=tweak_data.menu.pd2_large_font_size, font=tweak_data.menu.pd2_large_font, color=tweak_data.screen_colors.text } )
