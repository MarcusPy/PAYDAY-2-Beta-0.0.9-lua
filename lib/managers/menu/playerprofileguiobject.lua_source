...
function PlayerProfileGuiObject:init( ws )
	...
	local player_text = panel:text( { font=font, font_size=font_size, text=tostring(managers.network.account:username() or managers.blackmarket:get_preferred_character_real_name()), y=10, color=tweak_data.screen_colors.text } )
	...
	local money_text = panel:text( { text=self:get_text( "menu_cash", { money=managers.money:total_string() } ), font_size=font_size, font=font, color=tweak_data.screen_colors.text } )
	...
	local total_money_text = panel:text( { text=self:get_text( "hud_offshore_account" )..": "..managers.experience:cash_string(managers.money:offshore()) , font_size=font_size, font=font, color=tweak_data.screen_colors.text } )
	...
	if skillpoints > 0 then
		skill_text = panel:text( { text=self:get_text( "menu_spendable_skill_points", { points=tostring(skillpoints) } ), font_size=font_size, font=font, color=tweak_data.screen_colors.text, layer=1 } )
	...
	-- if mastermind_ponts == -1 then
	--- 	mastermind_ponts = "    "
	-- else
		mastermind_ponts = string.format( "%02d", mastermind_ponts ) -- .. "%"
	-- end
	local mastermind_text = panel:text( { text=self:get_text( "menu_profession_progress", { profession=self:get_text("st_menu_mastermind"), progress=mastermind_ponts, num_skills=num_skills } ), font_size=font_size, font=font, color=tweak_data.screen_colors.text, y=10 } )
	...
	-- if enforcer_ponts == 0 then
	-- 	enforcer_ponts = "    "
	-- else
		enforcer_ponts = string.format( "%02d", enforcer_ponts ) -- .. "%"
	-- end
	local enforcer_text = panel:text( { text=self:get_text( "menu_profession_progress", { profession=self:get_text("st_menu_enforcer"), progress=enforcer_ponts, num_skills=num_skills } ), font_size=font_size, font=font, color=tweak_data.screen_colors.text } )
	...
	-- if technician_ponts == 0 then
	-- 	technician_ponts = "    "
	-- else
		technician_ponts = string.format( "%02d", technician_ponts ) -- .. "%"
	-- end
	local technician_text = panel:text( { text=self:get_text( "menu_profession_progress", { profession=self:get_text("st_menu_technician"), progress=technician_ponts, num_skills=num_skills } ), font_size=font_size, font=font, color=tweak_data.screen_colors.text } )
	...
	-- if ghost_ponts == 0 then
	-- 	ghost_ponts = "    "
	-- else
		ghost_ponts = string.format( "%02d", ghost_ponts ) -- .. "%"
	-- end
	local ghost_text = panel:text( { text=self:get_text( "menu_profession_progress", { profession=self:get_text("st_menu_ghost"), progress=ghost_ponts, num_skills=num_skills } ), font_size=font_size, font=font, color=tweak_data.screen_colors.text } )
