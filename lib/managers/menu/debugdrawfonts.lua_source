...
function DebugDrawFonts:init( ws )
	...
	local macroes_of_texts = {} -- { BTN_Y="[F]", BTN_INTERACT="[G]", BTN_GADGET="[Q]" }
	...
	left_side:rect( { color=Color.black, alpha=0.4 } )
	
	local right_side = self._panel:panel()
	right_side:set_size( width/2-5, height )
	right_side:set_righttop( self._panel:w()-40,40 )
	right_side:rect( { color=Color.white, alpha=0.4 } )
	
	
	local blur = self._panel:bitmap( { texture="guis/textures/test_blur_df", w=self._panel:w(), h=self._panel:h(), render_template="VertexColorTexturedBlur3D", layer=-1 } )
	...
	for i, side in pairs( sides ) do
		...
		for _, font_data in ipairs( fonts ) do
			local text = side:text( { font=font_data[1], font_size=font_data[2], x=x, y=y, text=localized_text, layer=2, wrap=true, word_wrap=true, w=side:w()-2*x } )
			if i == 2 then
			...
			local _, _, tw, th = text:text_rect()
			text:set_size( tw, th )
			side:rect( { color=i==1 and Color.white or Color.black, alpha=0.6, layer=1 } ):set_shape( text:shape() )
