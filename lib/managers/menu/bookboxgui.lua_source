...
function BookBoxGui:init( ws, title, config )
	...
	self._header_type = config.header_type or "event" -- "fit", "even"
...
function BookBoxGui:add_page( name, box_gui, visible )
	local panel = self._panel:panel( { name = name, x = 0, h = 20, w = 40, layer = 10 } )
	panel:rect( { name = "bg_rect", color = Color( 1, 0.5, 0.5, 0.5 ), layer=0 } )
		
	panel:text( { name = "name_text", text = string.upper( name ), font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, x = 0, y = 0,
									align="center", halign="center", vertical="center", hvertical="center",
									layer = 1 } )
	
	-- self._active_page_name = self._active_page_name or (visible and name)
...
function BookBoxGui:remove_page( name )
	...
	if self._active_page_name == name then
		...
		print( "change to", n )
		self:set_page( n )
		-- self._active_page_name = next( self._pages )
	end
...
function BookBoxGui:close()
	...
	-- print( "BookBoxGui:close()", inspect( self._pages ) )
	for name,page in pairs( self._pages ) do
...
function BookBoxGui:check_grab_scroll_bar( x, y )
	...
	--[[if BookBoxGui.super.check_grab_scroll_bar( self, x, y ) then
		return true
	end]]
...
function BookBoxGui:mouse_moved( x, y )
	...
	if self._text_box:child( "top_line" ):inside( x, y ) then
		pointer = pointer or "grab"
	end]]
