...
function MenuItemCharacterExpand:init( data_node, parameters )
	...
	local data_node = {
					type		= "MenuItemCharacterAction",
					name		= self._parameters.character_id.."repair",
					text_id		= "menu_repair",
					action_type = "repair",
	...
	local data_node = {
					type		= "MenuItemCharacterAction",
					name		= self._parameters.character_id.."buy_upgrades",
...
function MenuItemCharacterExpand:setup_gui( node, row_item )
	-- print( "MenuItemCharacterExpand:setup_gui", self._parameters.weapon_id )
	...
	row_item.character_name:set_left( h + 4 )]]
...
function MenuItemCharacterExpand:reload( row_item, node )
	-- print( "MenuItemCharacterExpand:reload", self:name() )
...
function MenuItemCharacterExpand:_set_row_item_state( node, row_item )
	if self:expanded() or row_item.highlighted then
		row_item.character_name:set_color( node.row_item_hightlight_color ) -- Color.black )
		row_item.character_name:set_font( (row_item.font and Idstring(row_item.font)) or tweak_data.menu.default_font_no_outline_id )
	else
		row_item.character_name:set_color( self:parameter( "owned" ) and self:parameter( "unlocked" ) and row_item.color or node.row_item_color ) -- Color( 1, 0.5, 0.5, 0.5 ) )
