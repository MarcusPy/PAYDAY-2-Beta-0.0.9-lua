...
function MenuItemExpand:init( data_node, parameters )
	...
	if( data_node ) then
		for _,c in ipairs( data_node ) do
			...
			-- <item>
			if( type == "item" ) then
				-- local item = nil -- CoreMenuItemOption.ItemOption:new( c )
...
function MenuItemExpand:_show_items( callback_handler )
	-- print( "MenuItemExpand:_show_items", self:name() )
...
-- Collapses the expanded gui items, updates visible items and then expands the new gui items.
...
function MenuItemExpand:on_item_position( row_item, node )
	...
	-- row_item.expand_line:set_top( row_item.expand_line:top() - 4 )
end
...
function MenuItemExpand:reload( row_item, node )
	if not row_item.expanded_indicator then
		self:_create_indicator( row_item, node ) -- NOT NICE
	end
	row_item.expand_line = row_item.expand_line or row_item.gui_panel:parent():bitmap( { texture = "guis/textures/headershadowdown", texture_rect = { 0, 4, 256, 60 }, layer = node.layers.items + 1, color = Color.white, w = row_item.gui_panel:w() + node.align_line_padding( node ), y = 100 } )
		
	row_item.expanded_indicator:set_visible( self:expanded() )
	row_item.expand_line:set_visible( self:expanded() )
	if self:expanded() then
		row_item.expanded_indicator:set_color( node.row_item_color ) -- Color( 1, 0.5, 0 ) )
		row_item.menu_unselected:set_color( node.row_item_color ) -- Color( 1, 0.5, 0 ) )
	else
		row_item.expanded_indicator:set_color( node.row_item_hightlight_color ) -- Color.white )
		row_item.menu_unselected:set_color( node.row_item_hightlight_color ) -- Color.white )
	end
...
function MenuItemExpand:_set_row_item_state( node, row_item )
	if self:expanded() or row_item.highlighted then
		...
		row_item.current_of_total:set_color( self:expanded() and row_item.color or node.row_item_hightlight_color )  -- Color( self:expanded() and 0 or 0.5, 0, 0, 0 ) )
...
-- Returns highest item index that is not excluded
...
-- Returns lowest item index that is not excluded
...
--[[function MenuItemExpand:next()
	...
	if self._current_index == self:_highest_item_index() then
		return
	end
	
	repeat 
		self._current_index =  self._current_index == #self._items and 1 or (self._current_index + 1)
	until not self._items[ self._current_index ]:parameters().exclude
	
	return true
end

function MenuItemExpand:previous()
	if not self._enabled then
		return
	end
	...
	if self._current_index == self:_lowest_item_index() then
		return
	end
		
	repeat
		self._current_index = self._current_index == 1 and #self._items or (self._current_index - 1)
	until not self._items[ self._current_index ]:parameters().exclude
	
	return true
end

function MenuItemExpand:left_arrow_visible()
	return self._current_index > self:_lowest_item_index() and self._enabled
end

function MenuItemExpand:right_arrow_visible()
	return self._current_index < self:_highest_item_index() and self._enabled
end

function MenuItemExpand:arrow_visible()
...
function MenuItemExpandAction:setup_gui( node, row_item )
	...
	local texture, rect = tweak_data.hud_icons:get_icon_data( self._parameters.action_type == "equip" and "icon_equipped" 
															or self._parameters.action_type == "repair" and "icon_repair"
															or self._parameters.action_type == "buy_upgrades" and "icon_addon"
															or self._parameters.action_type == "buy" and "icon_buy"
															or self._parameters.action_type == "attach_upgrade" and "icon_equipped"
															or self._parameters.action_type == "buy_upgrade" and "icon_buy" )
	
	row_item.action_icon = row_item.gui_panel:bitmap( { texture = texture, texture_rect = rect, layer = node.layers.items } )
	...
	if self._parameters.action_type == "repair" then
		...
		row_item.circlefill = row_item.gui_panel:bitmap( { visible = true, texture = texture, texture_rect = rect, layer = node.layers.items } )
		...
		row_item.repair_circle = row_item.gui_panel:bitmap( { visible = self._parameters.unlocked, texture = texture, texture_rect = rect, layer = node.layers.items + 1, color = self:_repair_circle_color( self._parameters.parent_item:condition() ) } )
...
function MenuItemExpandAction:reload( row_item, node )
	-- print( "  MenuItemMaskAction:reload", self:name() )
	-- print( "     self:parameters().parent_item", self:parameters().parent_item:name() )
	...
	elseif self._parameters.action_type == "repair" then
		self:parameters().parent_item:on_repair( node ) -- , self._parameters.parent_item:condition() )
		-- print( "MenuItemWeaponAction:reload repair", self._parameters.parent_item:condition() )
		local texture, rect = tweak_data.hud_icons:get_icon_data( "icon_circlefill"..self._parameters.parent_item:condition() )
...
--[[function MenuItemExpandAction:_set_row_item_state( node, row_item )
	if row_item.highlighted then
		...
		row_item.action_name:set_font( tweak_data.menu.default_font_no_outline_id )
	else
		row_item.action_name:set_color( row_item.color )
		row_item.action_name:set_font( tweak_data.menu.default_font_id )
	end
end]]
...
function MenuItemExpandAction:_set_row_item_state( node, row_item )
	if row_item.highlighted then
		...
		if self._parameters.action_type == "repair" and self:_at_max_condition() then
			row_item.action_name:set_color( node.row_item_hightlight_color ) -- Color( 1, 0.5, 0.5, 0.5 ) )
		else
			row_item.action_name:set_color( row_item.color )
		end
