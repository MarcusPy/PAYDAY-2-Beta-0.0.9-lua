...
function MenuItemWeaponExpand:setup_gui( node, row_item )
	-- print( "MenuItemWeaponExpand:setup_gui", self._parameters.weapon_id )
	...
	row_item.expanded_indicator = row_item.gui_panel:parent():bitmap ( { visible = false, color = Color.red, texture = "guis/textures/menu_selected", x = 0, y = 0, layer = node.layers.items - 1 } )
...
function MenuItemWeaponExpand:on_repair( node ) --, condition )
	local row_item = node:row_item( self )
...
function MenuItemWeaponExpand:reload( row_item, node )
	-- print( "MenuItemWeaponExpand:reload", self:name() )
