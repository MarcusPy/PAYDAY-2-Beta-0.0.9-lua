...
function MenuItemKitSlot:icon_and_description()
	...
	elseif self._parameters.category == "equipment" then
		...
		-- local name_id = tweak_data.text_id
...
function MenuItemKitSlot.clbk_msg_set_kit_selection( overwrite_data, msg_queue, rpc_name, peer_id, category, selection_name, slot )
	if msg_queue then
		...
		if item_index then
			msg_queue[ item_index ] = { rpc_name, peer_id, category, selection_name, slot }	-- We already had such a message. Overwrite
		else
			table.insert( msg_queue, { rpc_name, peer_id, category, selection_name, slot } )
...
function MenuItemKitSlot:setup_gui( node, row_item )
	...
	row_item.choice_text = row_item.choice_panel:text( {
				...
				x = node._right_align( node ), y = 0,
				align="left", halign="center", vertical="center",
				font = row_item.font,
	...
	-- menu_arrows
	row_item.arrow_left = row_item.gui_panel:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, color = Color( 0.5,0.5,0.5 ), visible = self:arrow_visible(), x = 0, y = 0, layer = node.layers.items } )
	row_item.arrow_right = row_item.gui_panel:bitmap( { rotation = 180, texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, color = Color( 0.5,0.5,0.5 ), visible = self:arrow_visible(), x = 0, y = 0, layer = node.layers.items } )
	-- row_item.arrow_left =row_item.choice_panel:polygon( { visible = self:arrow_visible(), x = 0, y = 0, w = w, h = h, triangles = { Vector3( w,0,0 ), Vector3( 0,h/2,0), Vector3( w,h,0) }, layer = node.layers.items, color = node.row_item_color:with_alpha( 1 ) } )
	-- row_item.arrow_right =row_item.choice_panel:polygon( { visible = self:arrow_visible(), x = 0, y = 0, w = w, h = h, triangles = { Vector3( 0,0,0 ), Vector3( w,h/2,0), Vector3( 0,h,0) }, layer = node.layers.items, color = node.row_item_color:with_alpha( 1 ) } )
	...
	row_item.description_panel = node.safe_rect_panel:panel( { w = node.item_panel:w() / 2, h = 112, visible = false } )
	...
	row_item.description_icon = row_item.description_panel:bitmap( { name = "description_icon", texture = icon, layer = node.layers.items, texture_rect = texture_rect, x = 0, y = 0, w = 48, h = 48 } )
	
	row_item.description_progress_text = row_item.description_panel:text( {
										font_size = node.font_size,
										x = 0, y = 0,
										align="left", halign="left", vertical="bottom",
										font = row_item.font,
										color = row_item.color,
										layer = node.layers.items,
	...
	row_item.progress_bg = row_item.description_panel:rect( {	
								-- x = 52 + w, y = h/2 - 11, w = 128, h = 22,
								x = 52 + w + 4, y = 0, w = 256, h = 22,
	...
	row_item.progress_bar = row_item.description_panel:gradient( {	
								...
								x = row_item.progress_bg:x() + 2, y = row_item.progress_bg:y() + 2, w = (row_item.progress_bg:w() - 4) * 0.11, h = row_item.progress_bg:h() - 4,
								align="center", halign="center", vertical="center",
								-- color = node.color,
								layer = node.layers.items,
							} )
	row_item.progress_text = row_item.description_panel:text( {
								...
								x = row_item.progress_bg:x(), y = 0, h = h, w = row_item.progress_bg:w(), -- A bit stupid to set y to this, but centered y doesn't float the boat in this case
								align="center", halign="center", vertical="center", valign="center",
								font = node.font,
								color = node.color,
								layer = node.layers.items + 1,
								text = ""..math.floor( 0.11*100 ).."%",
								render_template = Idstring("VertexColorTextured")
							} )
	...
	row_item.description_text = row_item.description_panel:text( {
										...
										x = 0, y = 0,
										align="left", halign="left", vertical="top",
...
function MenuItemKitSlot:_layout( node, row_item )
	...
	row_item.choice_panel:set_w( safe_rect.width - node._right_align( node ) ) -- 300
...
--	row_item.description_panel:set_left( node._left_align( node ) - row_item.choice_panel:w() )
	...
	-- row_item.progress_bar:set_w( row_item.progress_bg:w() * 
