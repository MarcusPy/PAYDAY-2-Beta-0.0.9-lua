...
function MenuItemClothingExpand:init( data_node, parameters )
	...
	local data_node = {
					type		= "MenuItemClothingAction",
					name		= self._parameters.clothing_id.."repair",
					text_id		= "menu_repair",
					action_type = "repair",
	...
	local data_node = {
					type		= "MenuItemClothingAction",
					name		= self._parameters.clothing_id.."buy_upgrades",
...
function MenuItemClothingExpand:setup_gui( node, row_item )
	-- print( "MenuItemClothingExpand:setup_gui", self._parameters.weapon_id )
	...
	row_item.clothing_name:set_left( h + 4 )]]
...
function MenuItemClothingExpand:reload( row_item, node )
	-- print( "MenuItemClothingExpand:reload", self:name() )
	...
	if self:expanded() then
		...
		row_item.menu_unselected:set_color( node.row_item_hightlight_color ) -- Color( 0.5, 0.5, 0.5 ) )
	end
...
function MenuItemClothingExpand:_set_row_item_state( node, row_item )
	if self:expanded() or row_item.highlighted then
		row_item.clothing_name:set_color( node.row_item_hightlight_color ) -- Color.black )
		row_item.clothing_name:set_font( (row_item.font and Idstring(row_item.font)) or tweak_data.menu.default_font_no_outline_id )
	else
