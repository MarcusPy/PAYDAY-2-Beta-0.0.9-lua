...
--[[function FriendsBoxGui:_create_friend_action_gui()
	local data = {}
	...
	data.button_list = { join_game, chat_button, profile_button, achievements_button, stats_button, view_character_button }
	data.focus_button = 1
		
	self._friend_action_gui = TextBoxGui:new( self._ws, nil, nil, data, { only_buttons = true, no_close_legend = true, w = 200, h = 78 + 6 * 24, x = 0, y = 0 } )
	self._friend_action_gui:set_layer( 20 )
	self._friend_action_gui:set_visible( false )
end]]

function FriendsBoxGui:_create_friend_action_gui_by_user( user_data ) -- user )
	...
	-- Skip join game if allready in that lobby (or even in any lobby?)
	-- Currently skip join game if in any lobby
	if not my_lobby_id and user_lobby_id and not user_data.payday1 then -- and (not my_lobby_id or user_lobby_id ~= my_lobby_id) then
	...
	if not offline then
		if managers.network.matchmake.lobby_handler and not user_lobby_id then
			local invite = {}
			invite.text = "Invite"
	...
	self._friend_action_gui = TextBoxGui:new( self._ws, nil, nil, data, { only_buttons = true, no_close_legend = true, w = 200, h = h, x = 0, y = 0 } )
	self._friend_action_gui:set_layer( self:layer() + 20 )
	-- self._friend_action_gui:set_visible( false )
end

function FriendsBoxGui:set_layer( layer )
	-- print( "[FriendsBoxGui:set_layer]", layer )
...
function FriendsBoxGui:update_friends()
	...
	-- for _,user in ipairs( Steam:coplay_friends() ) do
		...
		-- print( "rich_presence_status", rich_presence_status )
		...
		elseif state == "online" or state == "away" then
			main_state = "online"
			sub_state = state
			--[[if rich_presence_level ~= "" then
				main_state = "ingame"
			else
				main_state = ( not user:lobby() and math.random( 2 ) == 1 and main_state ) or "ingame"
			end]]
		else
			main_state = state
			sub_state = state
		...
		-- Test rich presence change
		if user:lobby() then
			-- print( user:name(), "user:lobby()", user:lobby() )
			local numbers = managers.network.matchmake:_lobby_to_numbers( user:lobby() )
			if #numbers > 0 then
				-- print( "numbers", inspect( numbers ) )
		...
		elseif rich_presence_status == "" then
			if main_state == "ingame" then
				-- sub_state = math.random( 2 ) == 1 and sub_state or "Crime.net"
			elseif main_state == "offline" then
				-- sub_state = math.random( 2 ) == 1 and sub_state or "Toilet"
			end
		else
		...
		if not self._users[ user:id() ] then
			self._users[ user:id() ] = { user = user, main_state = main_state, sub_state = sub_state, lobby = user:lobby(), level = rich_presence_level, payday1 = payday1 }
	...
	for _,user in pairs( self._users ) do
		if user.main_state ~= user.current_main_state then
			-- print( "CHANGED main_state", user.user:name(), user.main_state, "->", user.current_main_state  )
			if user.main_state == "online" then -- or user.main_state == "away" then	
			...
			elseif user.current_main_state == "online" then -- or user.current_main_state == "away" then
		...
		elseif user.sub_state ~= user.current_sub_state then
			-- print( "CHANGED sub_state", user.user:name(), user.sub_state, "->", user.current_sub_state  )
...
function FriendsBoxGui:_update_sub_state( user_data )
	...
	user_state:set_text( string.upper( user_data.current_sub_state ) ) -- Localized
...
function FriendsBoxGui:_layout_friends_panel()
	...
	-- print( "friends_panel", friends_panel:h() )
...
function FriendsBoxGui:_create_user( friends_panel, h, user, state, sub_state, level )
	...
	local color = 	state == "online" and self._online_color
					or state == "ingame" and self._ingame_color
					or self._offline_color
	
	local panel = friends_panel:panel( { name = user:id(), y = h, layer = 0 } )
	...
	local user_name = panel:text( { name = "user_name", text = user:name(), font = tweak_data.menu.default_font, font_size = self._default_font_size, x = 16 + avatar:right(), y = 0,
									align="left", halign="left", vertical="center", hvertical="center",
									color = color, layer = 0 } )
		local _,_,tw,th = user_name:text_rect()
		user_name:set_size( tw, th )
		
	local user_level = panel:text( { name = "user_level", text = level, font = tweak_data.menu.small_font_noshadow, font_size = tweak_data.menu.small_font_size, x = 16, y = math.round( 0 ),
		...
		-- user_level:set_debug( true )

	local user_state = panel:text( { name = "user_state", text = string.upper( sub_state ), font = tweak_data.menu.small_font_noshadow, font_size = tweak_data.menu.small_font_size, x = 16 + avatar:right(), y = math.round( 0 + th ),
	...
	-- panel:set_h( th + (state ~= "offline" and sh or 0) + 4 )
	panel:set_h( th + sh + 4 )
...
function FriendsBoxGui:mouse_pressed( button, x, y )
	if self._friend_action_gui and self._friend_action_gui:visible() then
		if self._friend_action_gui:in_info_area_focus( x, y ) then
			if button == Idstring( "0" ) then
				-- local focus_btn = self._friend_action_gui:get_focus_button()
	...
	if self:in_info_area_focus( x, y ) then
		if button == Idstring( "0" ) then
			local user_panel = self:_inside_user( x, y )
			if user_panel then
				-- print( user_panel:name() )
				-- print( inspect( self._users[ user_panel:name() ] ) )
				-- print( self._users[ user_panel:name() ].user:name(), self._users[ user_panel:name() ].lobby, self._users[ user_panel:name() ].user:lobby() )
				if self._users[ user_panel:name() ].current_lobby then
					...
					-- managers.menu:active_menu().renderer._crimenet_gui:goto_lobby( self._users[ user_panel:name() ].lobby )
				end
				if self._friend_action_user ~= self._users[ user_panel:name() ].user then
					...
					-- self._friend_action_gui:set_visible( true )
					self:_create_friend_action_gui_by_user( self._users[ user_panel:name() ] ) -- self._friend_action_user )
					-- self._friend_action_gui:set_visible( true )
					-- self._friend_action_gui:set_title( self._friend_action_user:name() )
					x = x + 16
					y = y - 16 -- the frame is higher then the buttons
...
-- Returns a user panel inside x,y if any
...
function FriendsBoxGui:_set_user_panels_state( x, y, panel, color )
	for _,user_panel in ipairs( panel:children() ) do
		...
		-- user_panel:child( "marker_rect" ):set_visible( inside )
		...
		-- user_panel:child( "marker_rect" ):set_color( user_panel:inside( x, y ) and Color( 1, 0.5, 0.5 ,0.5 ) or Color( 0.5, 0.5, 0.5 ,0.5 ) )
	end
end
