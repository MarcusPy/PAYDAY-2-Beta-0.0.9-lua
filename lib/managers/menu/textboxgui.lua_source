...
function TextBoxGui:add_background()
	-- self._background = self._ws:panel():rect( { name = "bg", color = Color.black:with_alpha(0.4), layer=0, blend_mode="normal", valign = "grow" } )
	self._background = self._ws:panel():bitmap( { name = "bg", texture = "guis/textures/test_blur_df", color = Color.white, alpha=0, layer=0, render_template="VertexColorTexturedBlur3D", w=self._ws:panel():w(), h=self._ws:panel():h(), valign = "grow" } )
	self._background2 = self._ws:panel():rect( { name = "bg", color = Color.black, alpha=0, layer=0, blend_mode="normal", valign = "grow" } )
...
function TextBoxGui:_create_text_box( ws, title, text, content_data, config )
	...
	local title_text = main:text( { name = "title", text = title or "none", layer = 1, wrap = false, word_wrap = false, visible = title and true or false,
										...
										align="left", halign="left", vertical="center", hvertical="top", x = 10, y = 10 } )
	...
	local info_area = main:panel( { name="info_area", x = 0, y = 0, w = main:w(), h = main:h() - h*2, layer = 0 } )
	...
	local scroll_panel = info_area:panel( { name = "scroll_panel", x = 10, y = math.round( h + 5 ), w = info_area:w() - 20, h = info_area:h(), layer = 1 } )
		...
		for _,stats in ipairs( stats_list ) do
			if stats.type == "bar" then
				...
				local text = panel:text( { text = stats.text, layer = 1, w = panel:w(), h = panel:h(), x = 4, y = -1, 
									font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size,
									align="left", halign="left", vertical="center", hvertical="center", blend_mode="normal", kern=0.0 } )
			...
			elseif stats.type == "mods" then
				...
				local mods_text = panel:text( { text = s, layer = 0, w = panel:w(), h = panel:h(), y = text:bottom() ,x = icon:right(), 
									font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, 
									align="left", halign="left", vertical="top", hvertical="top", blend_mode="normal", kern=0.0 } )
