...
function ElementNavObstacle:on_script_activated()
	...
	if not self._values.obstacle_list then
		self._values.obstacle_list = { { unit_id = self._values.obstacle_unit_id, obj_name = self._values.obstacle_obj_name } }
	end
	
	for _,data in ipairs( self._values.obstacle_list ) do
		if Global.running_simulation then
			table.insert( self._obstacle_units, { unit = managers.editor:unit_with_id( data.unit_id ), obj_name = data.obj_name } )
		else
			-- managers.worlddefinition.get_unit
			...
			if unit then
				...
				table.insert( self._obstacle_units, { unit = unit, obj_name = data.obj_name } )
			end
		end 
	end
...
function ElementNavObstacle:_load_unit( obj_name, unit )
	-- print( "ElementNavObstacle:_load_unit(", unit, obj_name )
...
function ElementNavObstacle:on_executed( instigator )
	...
	for _,data in ipairs( self._obstacle_units ) do
		...
		elseif not data.unit:get_object( data.obj_name ) then
			...
			-- print( "ElementNavObstacle", self._values.operation, data.unit, data.obj_name )
...
function ElementNavObstacle:load( data )
	-- print( "load ElementUnitSequenceTrigger", self._has_active_callback, self._values.obstacle_unit_id )
