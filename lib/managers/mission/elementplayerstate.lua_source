...
function ElementPlayerState:on_executed( instigator )
	...
	--[[if self._values.rule and self._values.rule ~= "none" then
		managers.player:toggle_player_rule( self._values.rule )
	else]]
	...
	if not self._values.use_instigator or instigator == player_unit then
		if not requires_alive_player or alive( player_unit ) then
