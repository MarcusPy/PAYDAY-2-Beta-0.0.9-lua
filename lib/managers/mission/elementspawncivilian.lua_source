...
function ElementSpawnCivilian:produce()
	...
	if self._values.state ~= "none" then
		if unit:brain() then
			local action_data = { type = "act", variant = self._values.state, body_part = 1, align_sync = true }
			local spawn_ai = { 
				init_state = "idle",
				objective = { type = "act", action = action_data, interrupt_dis = -1, interrupt_health = 1 }
			 }
			unit:brain():set_spawn_ai( spawn_ai )	-- this will wake up the brain extension
		else
	...
	else -- Don't want the bank manager to be considered here, I think
