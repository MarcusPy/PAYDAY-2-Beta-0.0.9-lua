...
--[[function ElementEquipment:client_on_executed( ... )
	self:on_executed( ... )
end]]
...
function ElementEquipment:on_executed( instigator )
	...
	if self._values.equipment ~= "none" then
		if instigator == managers.player:player_unit() then
			managers.player:add_special( { name = self._values.equipment, amount = self._values.amount } )
		else
			local rpc_params = { "give_equipment", self._values.equipment, self._values.amount }
