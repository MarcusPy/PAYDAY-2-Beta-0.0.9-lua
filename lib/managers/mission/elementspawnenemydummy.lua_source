...
ElementSpawnEnemyDummy._spawn_stance_types = { "neutral", "hostile", "combat" } --ntl, hos, cbt
...
function ElementSpawnEnemyDummy:produce( params )
	...
	if params then
		-- get_positions( self._values.position )
		...
		unit:brain():set_spawn_ai( spawn_ai ) -- enable the brain extension update
...
-- called from SpawnEnemyUnitElement to test-run the spawner. The unit has already been spawned.
...
function ElementSpawnEnemyDummy:_create_spawn_AI_parametric( stance, objective, spawn_properties )
	...
	return { init_state = "idle", stance = stance, objective = objective, params = { scan = true } }
end

function ElementSpawnEnemyDummy:_create_action_data( spawn_properties )
	local action_name = spawn_properties.spawn_action or spawn_properties.state -- first is for enemies second is for civilians
	if not action_name or action_name == "none" then
		return { type = "idle", body_part = 1, sync = true }
	else
		return { type = "act", variant = action_name, body_part = 1, blocks = { action = -1, walk = -1, hurt = -1, heavy_hurt = -1 }, align_sync = true }
	end
end
