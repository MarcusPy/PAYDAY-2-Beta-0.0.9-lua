...
function ElementSecurityCamera:on_executed( instigator )
	...
	if ai_state or self._values.apply_settings then
		settings = {
			...
			detection_delay = { self._values.detection_delay_min, self._values.detection_delay_max }
		}
	end
...
function ElementSecurityCamera:check_triggers( type, instigator )
	-- print( "ElementSecurityCamera:check_triggers", type )
...
-- HOW IS DESTROYED KNOW AT CLIENT AND DROPPED IN, SAVE/LOAD? (martin)
