...
function Xbox360AchievementsDialog:update( t, dt )
	if( self._show_time and ( self._show_time ~= t ) and not Application:is_showing_system_dialog() ) then	-- Only way to determine when user has closed the dialog is by polling the engine like this.
		self:done_callback()
	end
end
