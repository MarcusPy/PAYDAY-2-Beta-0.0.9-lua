...
function Xbox360SelectUserDialog:update( t, dt )
	if( self._show_time and ( self._show_time ~= t ) and not Application:is_showing_system_dialog() and not self._manager:_is_engine_delaying_signin_change() ) then	-- Only way to determine when user has closed the dialog is by polling the engine like this.
		self:done_callback()
	end
end
