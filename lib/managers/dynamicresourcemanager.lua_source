...
function DynamicResourceManager:is_ready_to_close()
	return not self._to_unload -- if we have stuff to unload then we cannot close
end

-----------------------------------------------------------------------------------

function DynamicResourceManager:load( resource_type, resource_name, package_name, complete_clbk )
	--print( "[DynamicResourceManager:load]", resource_type, resource_name, package_name, complete_clbk )
...
function DynamicResourceManager:unload( resource_type, resource_name, package_name, keep_using )
	--print( "[DynamicResourceManager:unload]", resource_type, resource_name, package_name, keep_using )
	--Application:stack_dump()
	local resource_type_key = resource_type:key()
	local resource_name_key = resource_name:key()
	...
	if ref_count == 0 then
		...
		if not next( self._dyn_resources[ package_name ][ resource_type_key ] ) then -- clear empty tables
		...
		table.insert( self._to_unload, { package_name = package_name, resource_type = resource_type, resource_name = resource_name, keep_using = keep_using } )
	else
		self._dyn_resources[ package_name ][ resource_type_key ][ resource_name_key ] = ref_count
	end
end
