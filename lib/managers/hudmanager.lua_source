...
function HUDManager:add_waypoint( id, data )
	...
	if data.distance then
		distance = waypoint_panel:text( { name = "distance"..id, text = "16.5", color = data.color or Color.white, font = tweak_data.hud.medium_font_noshadow, font_size = tweak_data.hud.default_font_size, align = "center", vertical = "center", w = 128, h = 24, layer = 0, blend_mode = data.blend_mode } )
		distance:set_visible( false )
	end
	
	local timer = data.timer and waypoint_panel:text( { name = "timer"..id, text = (math.round(data.timer) < 10 and "0" or "")..math.round(data.timer), font = tweak_data.hud.medium_font_noshadow, font_size = 32, align = "center", vertical = "center", w = 32, h = 32, layer = 0} )
	--[[if data.timer then
