...
ClientNetworkSession.HOST_REQUEST_JOIN_INTERVAL = 2 -- resend request every 2 sec
...
function ClientNetworkSession:request_join_host( host_rpc, result_cb )
	...
	-- request_rpc:request_join( self._local_peer:name(), self._local_peer:mask_set() or "remove", managers.dlc:dlcs_string() )
	...
	self._join_request_params = {
		...
		params = { self._local_peer:name(), managers.blackmarket:get_preferred_character(), managers.dlc:dlcs_string(), xuid, lvl, gameversion, join_req_id }
	}
	...
	--[[if not host_name then
		request_rpc:request_player_name()
	end]]
end
...
function ClientNetworkSession:on_join_request_reply( reply, my_peer_id, my_character, level_index, difficulty_index, state_index, server_character, user_id, mission, job_id_index, job_stage, alternative_job_stage, interupt_job_stage_level_index, xuid, sender )
	...
	if not ( self._server_peer and self._cb_find_game ) then	-- We are not interested in joining anything
		return
	end
	...
	if self._server_peer:ip() and sender:ip_at_index(0) ~= self._server_peer:ip() then
		...
		return -- This is not the host we wanted to join
	end
	...
	if SystemInfo:platform() == self._ids_WIN32 then
		if self._server_peer:user_id() and user_id ~= self._server_peer:user_id() then
			...
			return -- This is not the host we wanted to join
		else -- WIN32 steam. let server decide the protocol
	...
	if reply == 1 then
		...
		self._server_peer:set_id( 1 )	-- server has always id 1
		self._server_peer:set_in_lobby_soft( state_index == 1 )
		self._server_peer:set_synched_soft( state_index ~= 1 )
		if SystemInfo:platform() == Idstring( "PS3" ) then
			-- self._server_peer:set_name( "Player 1" )
			-- self._local_peer:set_name( "Player "..tostring( my_peer_id ) )
		end
		...
		-- INFO IF TO LOAD LEVEL OR JOIN LOBBY
...
function ClientNetworkSession:on_host_discovered( new_host, new_host_name, level_name, my_ip, state, difficulty )
	...
	if self._discovered_hosts then
		-- First check if we already know about this host
		local new_host_data = { rpc = new_host, host_name = new_host_name, level_name = level_name, my_ip = my_ip, state = state, difficulty = difficulty }
		local already_known
...
function ClientNetworkSession:on_server_up_received( host_rpc )
	...
	-- Network:reset_connection( host_rpc )
end
...
function ClientNetworkSession:is_host()	return false end

------------------------------------------------------------------

function ClientNetworkSession:is_client() return true end
...
------------------------------------------------------------------
-- We receive info about a peer we didn't know about. Maybe we are new or he is new
function ClientNetworkSession:peer_handshake( name, peer_id, peer_user_id, in_lobby, loading, synched, character, mask_set, xuid, xnaddr )
	...
	peer_user_id = SystemInfo:platform() == self._ids_WIN32 and peer_user_id or false -- no user_id for ps3
...
-- This function is only valid on PS3
...
-- Server told us that he has finished loading (our turn)
...
------------------------------------------------------------------
-- we have now a mutual connection with peer: other_peer_id
...
------------------------------------------------------------------
-- the other peer has received our handshake. send info
function ClientNetworkSession:on_peer_requested_info( peer_id )
	--print( "[ClientNetworkSession:on_peer_requested_info]", peer_id )
	...
	other_peer:send( "peer_exchange_info", self._local_peer:id() ) -- notify other peer that we have sent all info
end
...
function ClientNetworkSession:on_peer_save_received( event, event_data )
	...
	if not ( kit_menu and kit_menu.renderer:is_open() ) then
		return
	end
...
function ClientNetworkSession:on_load_complete()
	...
	-- we pause ourselves automatically after load and wait for host to unpause us
	--managers.network:game():on_drop_in_pause_request_received( self._local_peer:id(), self._local_peer:name(), true )
end
...
function ClientNetworkSession:_get_join_attempt_identifier()
	if not self._join_attempt_identifier then
		self._join_attempt_identifier = math.random( 1, 65536 ) -- for these numbers check definition of packet "request_join"
	end
