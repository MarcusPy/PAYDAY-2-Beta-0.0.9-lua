...
BaseNetworkSession._LOAD_WAIT_TIME = 3 -- how long we wait for all msgs to be sent before beginning to load
...
function BaseNetworkSession:init()
	...
	local my_user_id = SystemInfo:platform() == self._ids_WIN32 and Steam:userid() or false -- PS3 has no user id
	...
	self._soft_remove_peers = false	-- Table with peers to be removed once they are done sending
	
	--	self._dead_con_reports = nil
	
	-- self._trash_connections = nil -- All connections that do not belong to our peers are here. They get auto-removed after self.CONNECTION_TIMEOUT
...
function BaseNetworkSession:load( data )
	...
	self.update = self.update_skip_one	-- this is to allow the keep alive messages to start arriving after our loading session. otherwise connections timeout
...
-- THIS SHOULD ONLY BE USED WHEN NAME IS UNIQUE, AS FOR PSN
...
function BaseNetworkSession:add_peer( name, rpc, in_lobby, loading, synched, id, character, user_id, xuid, xnaddr )
	...
	self._peers[ id ] = peer
	--cat_print( "multiplayer_base", "[BaseNetworkSession:add_peer]", inspect( peer ), inspect( self._peers ) )
...
function BaseNetworkSession:_soft_remove_peer( peer )
	...
	self._soft_remove_peers[ peer:rpc():ip_at_index(0) ] = { peer = peer, expire_t = TimerManager:wall():time() + 1.5 }	-- Wait 1.5sec before discarding, to send final messages
end

------------------------------------------------------------------

function BaseNetworkSession:on_peer_left_lobby( peer )
	if peer:id() == 1 and self:is_client() and self._cb_find_game then -- We as a client is asking to join this server
...
function BaseNetworkSession:on_peer_left( peer, peer_id )
	...
	if peer_id == 1 and self:is_client() then
		if self._cb_find_game then -- We as a client is asking to join this server
			self:on_join_request_timed_out()
		else
...
function BaseNetworkSession:on_peer_lost( peer, peer_id )
	...
	if peer_id == 1 and self:is_client() then
		if self._cb_find_game then -- We as a client is asking to join this server
			...
			-- Skip server left message if where are leaving ourselfs
...
function BaseNetworkSession:update_skip_one()
	self.update = nil	-- resets the updator to "update"
...
function BaseNetworkSession:update()
	...
	if self._closing then
		if self:is_ready_to_close() then
			self._closing = false
			managers.network:queue_stop_network()
		end
	end
	
	self:upd_trash_connections( wall_time )	-- Check if it is time to reset a connection
...
function BaseNetworkSession:clbk_network_send( target_rpc, post_send )
	...
	if post_send then
		if self._soft_remove_peers and self._soft_remove_peers[ target_ip ] then
			...
			if not peer then	-- I don't know you. remove
		...
		if peer then
			peer:on_send() -- Send overwriteable packets etc
		end
	end
end
...
function BaseNetworkSession:is_ready_to_close()
	--[[if self._load_wait_timeout_t and TimerManager:wall():time() > self._load_wait_timeout_t then
		return true
	end]]
	for peer_id, peer in pairs( self._peers ) do
		...
		if not peer:rpc() then -- we make other clients co-clients when we get their ip address. After that point it is safe to load because the engine sends "keep alive".
...
function BaseNetworkSession:set_peer_loading_state( peer, state )
	...
	if not state and self._local_peer:loaded() then -- send to the newly loaded peer our lobby info
		if peer:ip_verified() then
...
function BaseNetworkSession:add_connection_to_trash( rpc )
	...
	if not self._trash_connections[ wanted_ip ] then
		...
		self._trash_connections[ wanted_ip ] = { rpc = rpc, expire_t = TimerManager:wall():time() + self.CONNECTION_TIMEOUT }
	end
end
...
------------------------------------------------------------------
-- called from NetworkManager:stop_network
...
------------------------------------------------------------------
-- dummy p2p steam message used to discover the IP address of other clients
...
function BaseNetworkSession:chk_send_connection_established( name, user_id, peer )
	if SystemInfo:platform() == Idstring( "PS3" ) then
		...
		if connection_info.dead then
			if peer:id() ~= 1 then -- Skip sending to server about server
		...
		self:remove_connection_from_trash( rpc )
		self:remove_connection_from_soft_remove_peers( rpc )
	else -- SystemInfo:platform() == Idstring( "WIN32" ) then
...
function BaseNetworkSession:resolve_new_peer_rpc( new_peer, incomming_rpc )
	...
	local new_peer_ip_address = Network:get_ip_address_from_user_id( new_peer:user_id() ) -- returns string
	print( "new_peer_ip_address", new_peer_ip_address )
	if new_peer_ip_address then
		local new_peer_ip_address_split = string.split( new_peer_ip_address, ":" ) --table
		...
		if string.begins( new_peer_ip, "192.168." ) then
			...
			connect_port = NetworkManager.DEFAULT_PORT -- We have the same ip address. use internal port
		end
