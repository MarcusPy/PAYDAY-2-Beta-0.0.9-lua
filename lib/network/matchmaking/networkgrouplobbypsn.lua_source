--[[-----------------------------------------------------------------------------------------------
 ...
 Group lobby abstraction layer

 To do:

 create_group_lobby()
	Everyone will create a group lobby when they open the multiplayer menu. For XBL this means
	that your friends can see that you're in the BC group lobby, about to start a game. You
	can invite friends to the group lobby here. For Rendezvous it means we'll create a
	gathering which you can invite people to join.
	This function has been made asynchronous and will call the callback "created_group" when
	successfull or "create_group_failed" if failed (No connection to Xbox LIVE for example)

 join_group_lobby(id)
	If you've received an invite you will have to join the lobby through this method

 leave_group_lobby()
	Leave the current group. This will trigger an asynchronous operation that when finished
	will call the callback registered with the "left_group" callback.

 set_join_enabled(enabled)
	When join is enabled other players can join the group. Method of joining is different
	for each platform. On XBL join can be done via invites or join via presence. On PS3
	and RDV join can be done by invite.

 send_group_lobby_invite(user_id)
	Send an invite to a person on your friends list

 accept_group_lobby_invite(group_id, accept)
 	Notify the server if the user accepts the invite or not.

 send_game_id(id)
	Broadcast the game ID received by find_game() to all connected clients in the group lobby

 register_callback(event, callback)
	event: string
		The event that the function should be linked to. Events:
			"receive_group_lobby_invite"	: When a group lobby invite have been received
											  console. We need to ask the user if he wants
											  to join the group.
			"accepted_group_lobby_invite"	: When the user have accepted to join a group.
											  We separate the above and this callback because
											  on XBL the Xbox UI handles the accepting, the
											  game will only receive a callback when the user
											  have accepted an invite. At this point the
											  client should join the group.
			"create_group_failed"			: When we failed to create a group
			"created_group"		: Called when the group has been created
			"player_joined"		: Any player have joined the group. This is called for both
								  local and remote players.
			"player_left"		: A remote player have left the group.
			"receive_game_id"	: When the group leader has found a game he will distribute
								  the game ID to all the group members. Every group member
								  will get this callback.
			"left_group"		: When the asynchronous function leave_group has finished
								  its operations this callback is called
			"unknown_error"		: When something unexpected happens. Could be anything.

	callback: function
		The function to use

 start_game()
 	When the game is started no one should be able to join your group (up for discussion)
 	Maybe we can allow people to join during a game but when they've joined they'll be placed
 	in a waiting room, or maybe spectate 
 	XBL needs this function call for "accurate ranking".

 end_game()
	Allow people to join the group again

 is_group_leader(): boolean
 	Returns true if this client is the group leader, false otherwise
 	We need to know this because only the group leader can matchmake, and the leader
 	will have to distribute the found game ID to all the other group members

 get_leader_rpc()
	Return the RPC to the group host

 get_members_rpcs()
	Returns a table of the RPC's to all members of this group EXCEPT the group host
...
function NetworkGroupLobbyPSN:create_group_lobby()
	...
	local session_created = function(roomid) managers.network.group:_created_group_lobby(roomid) end
...
function NetworkGroupLobbyPSN:join_group_lobby( room_info )
	...
	local f = function(...) self:_join_invite(...) end
...
function NetworkGroupLobbyPSN:go_to_lobby()
	-- If registret go to lobby callback call it ( game registers it if lost connection and lobby member )
	if( self._callback_map[ "go_to_lobby" ] ) then
		self:_call_callback( "go_to_lobby" )
	else	-- Else leave lobby
...
function NetworkGroupLobbyPSN:send_return_group_lobby()
	...
	-- Setup a longer wait time on bundle as the times when returning to lobby seems to be close to random
	local timeout = 40
	if Application:bundled() then
		-- If we are running bundled we can use a lot shorter timeout
		timeout = 15
	end	
...
function NetworkGroupLobbyPSN:find( playerid )
	-- Loop all to remove the correct player in the group.
	for k,v in pairs( self._players ) do
...
function NetworkGroupLobbyPSN:send_game_id(id,private,created)
	if( created and created == true ) then --reserve slots for your local group members
		for k, v in pairs( self._players ) do
...
-- Returns RPC's to all group members but not the group host
...
function NetworkGroupLobbyPSN:_created_group_lobby( room_id )
	...
	PSN:set_matchmaking_callback( "session_created", function() end )
...
function NetworkGroupLobbyPSN:_register_player( name, pnid, group, rpc, is_server )
	...
	if( is_server and is_server == true ) then
		...
		self:_call_callback( "player_joined", {player_id=managers.network.account:player_id(), group_id=tostring( self._room_id ), name=managers.network.account:username(), rpc=Network:self( "TCP_IP" )} )
	end
...
function NetworkGroupLobbyPSN:_unregister_player( pnid, is_server, rpc )
	-- If it's the server unregistering, leave the group
...
-- Called if we leave a game via the ingame menu
