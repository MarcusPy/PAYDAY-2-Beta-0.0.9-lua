...
-----------------------------------------------------------------------------------
-- alert_data: { alert_nature, epicenter, effective_radius, alerting_unit } or { alert_nature, cylinder_bottom, cylinder_top, cylinder_radius, alerting_unit }
...
function HuskCopBrain:on_long_dis_interacted( amount, aggressor_unit )
	amount = math.clamp( math.ceil( amount * 10 ), 0, 10 )
	self._unit:network():send_to_host( "long_dis_interaction", amount, aggressor_unit ) -- We use the same rpc as for cop intimidation. we detect on server side that its actually an interact
end
