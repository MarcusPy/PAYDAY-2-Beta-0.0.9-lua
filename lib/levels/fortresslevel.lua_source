...
function FortressLevel:init()
	...
	self._spawn_index = 1
	self._spawn_map = {}
	table.insert( self._spawn_map, { name = Idstring( "units/characters/dummy_duel/dummy_duel" ), func = self.spawn_dummy } )
...
function FortressLevel:update( t, dt )
	if self._spawn_btn_presses then
		self:spawn_dummy()
		--if t - self._spawn_btn_press_t > 1 then
...
--[[
function FortressLevel:paused_update( t, dt )
end
]]
...
function FortressLevel:_spawn_enemy_group()
	...
	local dummies = { amount = self._spawn_btn_presses }
	group_data[ "units/characters/drone/drone" ] = dummies
	
	managers.spawn:spawn_enemy_group( group_data )
	]]
end
