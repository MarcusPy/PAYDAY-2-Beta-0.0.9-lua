...
function MenuSetup:init_game()
	...
	if( not Application:editor() ) then
		local event_id, checkpoint_index
		local level, level_class_name, mission, world_setting
		local intro_skipped
		...
		if( level ) then	-- Started a level with exe arguments:
...
function MenuSetup:init_managers( managers )
	...
	managers.network = NetworkManager:new( "NetworkGame" ) -- will be changed to NetworkGameMenu later..
end
...
function MenuSetup:update_wait_for_savegame_info( t, dt )
	...
	if managers.savefile:fetch_savegame_hdd_space_required() then	-- -1 means we have already 
