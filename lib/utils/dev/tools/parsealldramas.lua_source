...
function ParseAllDramas:start_parsing()
	...
	self._panel:rect( { color=Color(0.0, 0.0, 0.0) } )
	self._text = self._panel:text( { name="text", font=tweak_data.menu.pd2_medium_font, font_size=tweak_data.menu.pd2_medium_font_size, layer=1, text="", align="center" } )
	self._text:set_text( "0/".. tostring(#self._dramas) )
	
	self._parsed_sound_events = {}
	
	
	local update_anim = function( panel )
