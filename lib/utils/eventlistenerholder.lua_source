...
function EventListenerHolder:_set_new( key, event_types, clbk )
	...
	if self._trash then	-- If the listener was removed while calling unqueue the deletion
		self._trash[ key ] = nil
	end 
end
...
function EventListenerHolder:_append_new_additions()
	if self._additions then
		...
		for key, new_entry in pairs( self._additions ) do
			for _, event in ipairs( new_entry[2] ) do		-- new_entry looks like this: { clbk, event_types }
				listeners[ event ] = listeners[ event ] or {}
