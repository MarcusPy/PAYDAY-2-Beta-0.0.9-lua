...
function IngameMinigameState:init( game_state_machine )
	...
	self._text = self._panel:text{ name="text", text="Minigame", font="fonts/font_fortress_22", font_size = 24, color=Color.black, layer=1 }
	
	self._rect = self._panel:rect{ x=Application:screen_resolution().x * 0.25, y=Application:screen_resolution().y * 0.25, w=Application:screen_resolution().x * 0.5, h=Application:screen_resolution().y * 0.5, color=Color.white:with_alpha(0) }
