...
function BootupState:old()
	self._play_data_list = {
								{ visible = (not is_win32), gui = Idstring( "guis/autosave_warning" ), width = 600, height = 200, can_skip = false, fade_in = 1.25, duration = is_win32 and 0 or 6, fade_out = 1.25 },
								{ visible = show_esrb, layer = 1, texture = "guis/textures/esrb_rating", width = esrb_y*2, height = esrb_y, can_skip = has_full_game, fade_in = 1.25, duration = show_esrb and 6.5 or 0, fade_out = 1.25 },
								{ layer = 1, texture = "guis/textures/soe_logo", width = 256, height = 256, padding = 200, can_skip = can_skip, fade_in = 1.25, duration = 6, fade_out = 1.25 },
								{ layer = 1, text = legal_text, font=tweak_data.menu.default_font, font_size=24, wrap=true, word_wrap=true, vertical = "center", width = safe_rect_pixels.width, height = safe_rect_pixels.height, padding = 200, can_skip = can_skip, fade_in = 1.25, duration = 6, fade_out = 1.25 },
								-- { layer = 1, video = "movies/sony_logo", width = res.x, height = res.y, padding = 200, can_skip = true --[[, fade_in = 1.25, duration = 5, fade_out = 1.25]] },
								--[[{ layer = 1, texture = "guis/textures/esrb_rating", width = 256, height = 256, padding = 200, can_skip = true, duration = 5 },]]
								{ layer = 1, video = "movies/company_logo", width = res.x, height = res.y, padding = 200, can_skip = true --[[, fade_in = 1.25, duration = 5, fade_out = 1.25]] },
								{ layer = 1, video = "movies/game_logo", width = res.x, height = res.y, padding = 200, can_skip = true --[[, fade_in = 1.25, duration = 5, fade_out = 1.25]] },
								-- { texture = "guis/textures/seven_figures_logo", width = 256, height = 256, padding = 150, can_skip = true, fade_in = 1.25, duration = 5, fade_out = 1.25 },
								-- { texture = "guis/textures/stonecold_logo", width = 1024, height = 128, padding = 50, can_skip = true, fade_in = 1.25, duration = 7, fade_out = 1.25 },
						   }
end
...
function BootupState:setup()
	...
	-- Don't need ESRB for disc games
	local show_esrb = false	-- Application:region() == Idstring( "us" )
	--[[if is_x360 then
		show_esrb = XboxLive:game_region() == "north_america"
	end ]]
	...
	-- self._workspace:set_debug( true )
	...
	self._play_data_list = {
								{ visible = (not is_win32), layer = item_layer, gui = Idstring( "guis/autosave_warning" ), width = 600, height = 200, can_skip = false, fade_in = 1.25, duration = is_win32 and 0 or 6, fade_out = 1.25 },
								{ visible = show_esrb, layer = item_layer, texture = "guis/textures/esrb_rating", width = esrb_y*2, height = esrb_y, can_skip = has_full_game, fade_in = 1.25, duration = show_esrb and 6.5 or 0, fade_out = 1.25 },
								-- { layer = item_layer, texture = "guis/textures/soe_logo", width = 256, height = 256, padding = 200, can_skip = can_skip, fade_in = 1.25, duration = 6, fade_out = 1.25 },
								{ layer = item_layer, text = legal_text, font=tweak_data.menu.pd2_medium_font, font_size=24, wrap=true, word_wrap=true, vertical = "center", width = safe_rect_pixels.width, height = safe_rect_pixels.height, padding = 200, can_skip = can_skip, fade_in = 1.25, duration = 6, fade_out = 1.25 },
								-- { layer = item_layer, video = "movies/company_logo", width = res.x, height = res.y, padding = 200, can_skip = true --[[, fade_in = 1.25, duration = 5, fade_out = 1.25]] },
								{ layer = item_layer, video = "movies/game_intro", width = res.x, height = res.y, padding = 200, can_skip = true --[[, fade_in = 1.25, duration = 5, fade_out = 1.25]] },
							-- 	{ layer = item_layer, video = "movies/game_logo", width = res.x, height = res.y, padding = 200, can_skip = true --[[, fade_in = 1.25, duration = 5, fade_out = 1.25]] },
						   }
	
	self._full_panel = self._full_workspace:panel()
	self._panel = self._workspace:panel()
	self._full_panel:rect( { visible = false, color = Color.red, layer = 0 } )
	-- self._panel:bitmap( { visible = true, texture = "guis/textures/esrb_rating", layer = 0 } )
	...
	if( self._play_data ) then
		...
		if self._play_data.gui then
			if( self._play_data.width / self._play_data.height > res.x / res.y ) then
				...
				-- height = res.y - padding * 2
				-- width = self._play_data.width * height / self._play_data.height
		...
		local x = ( self._panel:w() - width ) / 2 -- ( res.x - width ) / 2
		local y = ( self._panel:h() - height ) / 2 -- ( res.y - height ) / 2
		...
		if( self._play_data.video ) then
			...
			-- self._gui_obj:set_size( res.y/m, res.y )
