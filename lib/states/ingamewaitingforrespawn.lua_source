...
function IngameWaitingForRespawnState:init( game_state_machine )
	...
	self._rot = Rotation()					-- mutable rotation used in camera calculations
	self._vec_target = Vector3()		-- mutable vector used in camera calculation, for target
	self._vec_eye = Vector3()				-- mutable vector used in camera calculation, for eye position
	self._vec_dir = Vector3()  		 	-- mutable vector used in camera calculation, for raycast direction
end
...
function IngameWaitingForRespawnState:_setup_sound_listener()
	...
	self._sound_check_object = managers.sound_environment:add_check_object( { object = self._camera_object, active = true, primary = true } )
end
...
function IngameWaitingForRespawnState:update( t, dt )
	if self._player_state_change_needed and not alive(managers.player:player_unit()) then
		...
		managers.player:set_player_state( "standard" )	-- the movement state the player unit is going to start in
	end
	...
	if self._auto_respawn_t then
		...
		if self._auto_respawn_t < t then
			self._auto_respawn_t = nil
			self:_begin_game_enter_transition() -- start fading to black
		end
	elseif self._ready_to_spawn_t then
		if self._ready_to_spawn_t < t then
			IngameWaitingForRespawnState.request_player_spawn() -- we have faded to black. request player spawn
		end
	end
...
-- local mvec3_set_x = mvector3.set_x				-- not in use
-- local mvec3_set_y = mvector3.set_y				-- not in use
-- local mvec3_set_z = mvector3.set_z				-- not in use
local mvec3_x = mvector3.x
local mvec3_y = mvector3.y
-- local mvec3_z = mvector3.z								-- not in use
...
function IngameWaitingForRespawnState:_upd_watch( t, dt )
	...
	if self._spectator_data.watch_u_key then
		...
		mvec3_set( self._vec_dir, self._controller:get_input_axis( "look" ) )		-- self._vec_dir  doubles as axis input
		...
		if mvec3_length( self._vec_dir ) > 0.1 then
			...
			mvec3_cross( self._vec_target, math_up, self._fwd )		-- doubles as direction
			...
			if( rot ~= 0 ) then
				...
				mvec3_rotate_with( self._fwd, self._rot )
						...
						-- Prev math
		-- local target = watch_u_record.unit:movement():m_head_pos()  -- watch_u_head_pos
		-- local eye_pos = target - self._fwd * 150
		-- local eye_rot = Rotation( self._fwd, math.UP )
		-- -- target = target:with_z( eye_pos.z )												-- no longer used when we are able to move camera up and down
		
		watch_u_head:m_position( self._vec_target )								-- target
		
		mvec3_set( self._vec_eye, self._fwd )									-- eye_pos
		mvec3_multiply( self._vec_eye, 150 )
		mvec3_negate( self._vec_eye )
		mvec3_add( self._vec_eye, self._vec_target )
		
		mrot_set_look_at( self._rot, self._fwd, math_up )		-- eye_rot
		
		-- mvec3_set_z( self._vec_target, mvec3_z( self._vec_eye ) )
		
		
		local col_ray = World:raycast( "ray", self._vec_target, self._vec_eye, "slot_mask", self._slotmask )
		local dis_new
		-- local dir_new 		-- now self._vec_dir
		
		if col_ray then
						-- Prev math
			-- dir_new = col_ray.ray
			-- dis_new = math.max(col_ray.distance - 30, 0)		-- same
		...
		-- TODO : Fix lerp back if dis_curr < dis_new
			-- Fix so that camera actually moves in raychecked space, so that it does not clip the ceiling etc
			-- Fix so that the screen blurs when it moves quickly inwards
			-- Fix so that the screen blurs when dis is too close
		...
		if not managers.hud:visible( self.GUI_SPECTATOR_FULLSCREEN ) then
			managers.hud:show( self.GUI_SPECTATOR_FULLSCREEN )
		end
	end
	
	--[[for u_key, u_data in pairs( self._spectator_data.teammate_records ) do
		if u_key == self._spectator_data.watch_u_key then
			Application:draw_sphere( u_data.m_pos, 60, 1,0,0 )
		else
			Application:draw_sphere( u_data.m_pos, 60, 0,1,0 )
		end
	end]]
end
...
function IngameWaitingForRespawnState:at_enter()
	...
	--managers.hud:script( self.GUI_SPECTATOR ).timer:set_visible( false )
...
function IngameWaitingForRespawnState:_refresh_teammate_list()
	...
	-- Remove invalid teammates from indexed list
	...
	while i > 0 do
		...
		if not teammate_data then
			--print( "[IngameWaitingForRespawnState:_refresh_teammate_list] unit key not verified", u_key )
			table.remove( teammate_list, i )	-- this teammate is not active
			if u_key == self._spectator_data.watch_u_key then
		...
		for u_key, u_data in pairs( all_teammates ) do
			...
			for i_key, test_u_key in ipairs( teammate_list ) do
				if test_u_key == u_key then
					add = false	-- we already have this unit indexed
					break
				end
			end
			if add then
				--print( "[IngameWaitingForRespawnState:_refresh_teammate_list] unit key added", u_key )
				table.insert( teammate_list, u_key )
			end
		end
	end
...
--	managers.hud:script( self.GUI_SPECTATOR ).timer:set_visible( true )
	managers.hud:set_custody_can_be_trade_visible( false )
