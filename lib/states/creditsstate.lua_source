...
function CreditsState:setup()
	...
	meta_credits.has_next = function( self )
		return not (not self[self._index])
	end
	meta_credits.get_next = function( self, index, sub_index )
	...
	meta_credits.step = function( self )
	...
	meta_credits.next = function( self )
	...
	--self._workspace:panel():rect( { color = Color.black, w = res.x, h = res.y } )
	self._text_panel = self._workspace:panel():panel( { h=0, y=self._workspace:panel():h(), layer=1 } )
...
function CreditsState:update( t, dt )
	...
	if( alive( self._text_panel:child(0) ) and self._text_panel:child(0):world_bottom() <= 0 ) then
	...
	elseif( self._text_panel:bottom() <= 0 ) then
		self:continue()
	end
end
